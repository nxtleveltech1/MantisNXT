#!/usr/bin/env node
/**
 * Execute migration 0015 using project's database infrastructure
 */
...
async function executeMigration() {
  ...
  const migrationPath = join(dirname(__dirname), 'migrations', '0015_soh_pricing_with_ai.sql');
  ...
  try {
    // Split the migration into statements and execute
    const statements = migrationSQL
      .split(/;\s*$\s*/m)
      .filter(stmt => stmt.trim() && !stmt.trim().startsWith('--'))
      .filter(stmt => !stmt.includes('VERIFICATION QUERIES'));
    ...
    console.log('✅ pricing_automation_config table:', configTable[0].count === '1' ? 'created' : 'not found');
    ...
    console.log('✅ pricing_recommendation_queue view:', queueView[0].count === '1' ? 'created' : 'not found');
    ...
    console.log('✅ ai_service_type enum:', aiServiceType.length > 0 ? 'pricing_recommendation added' : 'not added');
