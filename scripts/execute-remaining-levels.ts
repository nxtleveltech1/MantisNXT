...
for (const level of levels) {
  ...
  console.log(`Level ${level}: ${statements.length} statements in ${Math.ceil(statements.length / batchSize)} batches`);
}

console.log(`\nTotal batches to execute: ${batches.length}`);
console.log(`\nFirst batch of Level 3 (statements 21-40):`);
console.log(JSON.stringify(batches.find(b => b.level === 3 && b.batchNum === 2)?.statements.slice(0, 2), null, 2));
