...
async function main() {
  ...
  const evalTool =
    tools.find(t => t.name.toLowerCase().includes('evaluate')) ||
    tools.find(t => t.name.toLowerCase().includes('script')) ||
    tools[1];
  ...
  if (listCount !== total && listCount > 0)
    throw new Error(`SSOT parity mismatch: dashboard=${total} suppliers=${listCount}`);

  console.log(
    `[E2E] SSOT parity OK. suppliers=${listCount}, dashboard.total=${total}, dashboard.active=${active}`
  );
}
