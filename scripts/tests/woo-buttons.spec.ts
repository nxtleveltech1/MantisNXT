...
async function main() {
  ...
  let res = await post(
    '/api/v1/integrations/woocommerce/preview/refresh',
    { entities: ['customers'] },
    { 'x-org-id': ORG }
  );
  ...
  res = await get(
    `/api/v1/integrations/woocommerce/table?entity=customers&orgId=${ORG}&page=1&pageSize=10`
  );
  ...
  res = await post(
    '/api/v1/integrations/woocommerce/select',
    { entity: 'customers', ids, selected: true },
    { 'x-org-id': ORG }
  );
  assert(res.ok, 'select failed');

  console.log('▶ Testing selected sync (customers)');
  res = await post('/api/v1/integrations/woocommerce/sync/selected', {
    org_id: ORG,
    entity: 'customers',
    ids,
  });
  ...
  res = await get(
    `/api/v1/integrations/woocommerce/table?entity=products&orgId=${ORG}&page=1&pageSize=10`
  );
  json = await res.json();
  const pids = json.data
    .slice(0, 2)
  ...
  res = await post(
    '/api/v1/integrations/woocommerce/schedule/customers',
    {},
    { 'x-org-id': ORG, 'x-admin': 'true' }
  );
