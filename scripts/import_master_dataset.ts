...
import type { PricelistRow, Supplier, PricelistUploadRequest } from '../src/types/nxt-spp';
...
function detectColumnMappings(headers: string[]): ColumnMapping {
  ...
  for (const header of headers) {
    ...
    // SKU/Code mappings
    if (['sku', 'code', 'item code', 'part number', 'supplier sku'].includes(lowerHeader)) {
      mapping.supplier_sku = header;
    }
    // Name mappings
    else if (
      ['name', 'description', 'product name', 'item name', 'product'].includes(lowerHeader)
    ) {
      mapping.name = header;
    }
    // Price mappings
    else if (['price', 'unit price', 'cost', 'selling price', 'list price'].includes(lowerHeader)) {
    ...
    // Category mappings
    else if (['category', 'group', 'type', 'classification'].includes(lowerHeader)) {
      mapping.category_raw = header;
    }
    // UOM mappings
    else if (['uom', 'unit', 'unit of measure', 'measure'].includes(lowerHeader)) {
      mapping.uom = header;
    }
    // Barcode mappings
...
function transformRowToPricelistRow(
  ...
  return {
    ...
    uom: String((mapping.uom ? row[mapping.uom] : undefined) || 'EACH').trim(),
    pack_size: undefined,
    price: parseFloat(String(row[mapping.price] || '0').replace(/[^0-9.-]/g, '')) || 0,
    currency: currency,
    category_raw: mapping.category_raw ? String(row[mapping.category_raw] || '').trim() : undefined,
    vat_code: undefined,
    barcode: mapping.barcode ? String(row[mapping.barcode] || '').trim() : undefined,
...
async function importMasterDataset(): Promise<ImportStats> {
  ...
  try {
    ...
    for (const sheetName of targetSheets) {
      ...
      console.log(`📊 Found ${jsonData.length} rows in worksheet: ${sheetName}`);
      ...
      if (!mapping.supplier_sku || !mapping.name || !mapping.price) {
        throw new Error(`Missing required columns. Found: ${JSON.stringify(mapping)}`);
      }
      ...
      if (supplierColumn) {
        ...
        for (const row of jsonData) {
          const supplierName = String(row[supplierColumn] || 'Unknown Supplier').trim();
      ...
      console.log(`🏢 Processing ${Object.keys(supplierGroups).length} suppliers`);
      ...
      for (const [supplierName, rows] of Object.entries(supplierGroups)) {
        try {
          ...
          for (let i = 0; i < rows.length; i++) {
            ...
            if (!pricelistRow.supplier_sku || !pricelistRow.name || pricelistRow.price <= 0) {
          ...
          for (let i = 0; i < pricelistRows.length; i += BATCH_SIZE) {
            ...
            const progress = Math.round(((i + batch.length) / pricelistRows.length) * 100);
            console.log(
              `📊 Progress: ${progress}% (${i + batch.length}/${pricelistRows.length} rows)`
            );
          }
          ...
          if (validationResult.status === 'invalid') {
            console.warn(`⚠️  Validation failed for ${supplierName}:`, validationResult.errors);
          ...
          if (mergeResult.success) {
            ...
            console.error(`❌ Merge failed for ${supplierName}:`, mergeResult.errors);
          }
