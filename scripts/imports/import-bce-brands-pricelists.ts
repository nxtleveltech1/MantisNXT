...
const JUNE_FILE_PATH =
  'K:\\00Project\\MantisNXT - Uploads\\All files\\2025_BCE_Brands_Pricelist_June_Final.01.pdf';
const SEPTEMBER_FILE_PATH =
  'K:\\00Project\\MantisNXT - Uploads\\All files\\2025_BCE_Brands_Pricelist_September BC ELECTRONICS.pdf';
...
async function findSupplier(client: Client, supplierName: string): Promise<string | null> {
  const result = await client.query(
    `SELECT supplier_id, name, code 
     ...
     LIMIT 5`,
    [`%${supplierName.toLowerCase()}%`, `%bce brands%`, `%bc electronics%`]
  );
