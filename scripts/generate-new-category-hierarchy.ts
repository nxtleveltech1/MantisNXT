/**
 * Generate SQL migration for new streamlined category hierarchy
...
interface CategoryNode {
  ...
  path: string;
  slug: string;
  number: string; // Original numbering (e.g., "1.1.1")
}
...
function cleanCategoryName(text: string): string {
  // Remove numbering (1.1.1.1.1) and trim
  return text
...
function parseHierarchy(content: string): CategoryNode[] {
  ...
  const hierarchy: { [level: number]: { name: string; slug: string; path: string; number: string } } = {
    1: { name: '', slug: '', path: '', number: '' },
    2: { name: '', slug: '', path: '', number: '' },
    3: { name: '', slug: '', path: '', number: '' },
    4: { name: '', slug: '', path: '', number: '' },
    5: { name: '', slug: '', path: '', number: '' },
  };
  ...
  for (const rawLine of lines) {
    ...
    // Match numbered format: "1. Guitars" or "1.1 Electric Guitars" or "1.1.1 Solid-Body"
    // Pattern: Level 1 has format "1. Name" (dot after number)
    //          Sub-levels have format "1.1 Name" (no dot after last number, just space)
    let match = line.match(/^(\d+)\.\s+(.+)$/); // Level 1: "1. Name"
    if (match) {
      number = match[1];
      name = cleanCategoryName(match[2]);
      level = 1;
    } else {
      // Sub-levels: "1.1 Name" or "1.1.1 Name" etc.
      match = line.match(/^(\d+(?:\.\d+)+)\s+(.+)$/); // Has dots between numbers, space before name
      if (match) {
        number = match[1];
        name = cleanCategoryName(match[2]);
        // Determine level by counting dots in the number
        // Format: "1.1" = 1 dot = level 2, "1.1.1" = 2 dots = level 3, "1.1.1.1" = 3 dots = level 4, "1.1.1.1.1" = 4 dots = level 5
        // "15.1.4" = 2 dots = level 3, "15.1.4.1" = 3 dots = level 4
        const dotCount = (number.match(/\./g) || []).length;
        level = dotCount + 1; // "1.1" = 1 dot + 1 = level 2, "1.1.1" = 2 dots + 1 = level 3, "1.1.1.1" = 3 dots + 1 = level 4, "1.1.1.1.1" = 4 dots + 1 = level 5
      } else {
        // Try alternative format: "1.1 Name" (with dot after first number)
        match = line.match(/^(\d+)\.(\d+(?:\.\d+)*)\s+(.+)$/);
        if (match) {
          number = match[1] + '.' + match[2];
          name = cleanCategoryName(match[3]);
          const dotCount = (number.match(/\./g) || []).length;
          level = dotCount + 1;
        } else {
          // Debug: log lines that don't match
          if (line && line.trim() && !line.startsWith('--')) {
            // Only log if it looks like it might be a category line
            if (/^\d/.test(line)) {
    ...
    if (match) {
      // Allow up to level 5 (5 dots = level 5)
      if (level > 5) {
        console.warn(`Skipping category with level > 5: ${line}`);
        continue;
      }
    } else {
    ...
    hierarchy[level] = { name, slug, path: fullPath, number };
    // Clear deeper levels
    for (let l = level + 1; l <= 5; l++) {
      hierarchy[l] = { name: '', slug: '', path: '', number: '' };
    }
...
function generateSQL(categories: CategoryNode[]): string {
  ...
  sql.push('-- Migration: Seed New Streamlined Category Hierarchy');
  sql.push('-- Description: Seeds new streamlined category hierarchy replacing 309 old categories');
  sql.push('-- Generated: ' + new Date().toISOString());
  sql.push('');
  sql.push('BEGIN;');
  sql.push('');
  sql.push('-- Ensure unique constraint on path for conflict detection');
  sql.push('CREATE UNIQUE INDEX IF NOT EXISTS idx_core_category_path_unique ON core.category(path);');
  sql.push('');
  ...
  for (let level = 1; level <= 5; level++) {
    ...
    for (const cat of levelCategories) {
      ...
      sql.push(`  NOW()`);
      sql.push(`)`);
      sql.push(`ON CONFLICT (path) DO NOTHING;`);
      sql.push('');
    }
  }
...
// Main execution - accepts hierarchy content as input
function generateCategoryHierarchy(hierarchyContent: string, outputPath?: string): string {
  console.log('Parsing hierarchy...');
  const categories = parseHierarchy(hierarchyContent);
  ...
  for (let level = 1; level <= 5; level++) {
    ...
    const count = levelCats.length;
    if (count > 0) {
      console.log(`  Level ${level}: ${count} categories`);
      if (level <= 2 && levelCats.length > 0) {
        console.log(`    Examples: ${levelCats.slice(0, 3).map(c => c.name).join(', ')}`);
      }
    }
  }
  ...
  if (categories.length > 0) {
    ...
    categories.slice(0, 10).forEach(c => {
      console.log(`  L${c.level} (${c.number}): ${c.name} -> ${c.path}`);
    });
  }
...
// If run directly, expect hierarchy content from stdin or file
if (require.main === module) {
  const hierarchyContent = `1. Guitars, Basses & Amps
...
15.4.3 Tools & Testers (Multimeters/Soldering Kits)`;
  ...
  const outputFile = join(
    process.cwd(),
    'migrations',
    '0040_seed_new_category_hierarchy.sql'
  );

  generateCategoryHierarchy(hierarchyContent, outputFile);
}

export { generateCategoryHierarchy, parseHierarchy };
