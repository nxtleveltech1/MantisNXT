 ...
 */
function validateSecretStrength(secret: string, minEntropy: number): {
  valid: boolean
  entropy: number
  message: string
} {
...
function displaySecrets(secrets: GeneratedSecret[]): void {
  ...
  for (const secret of secrets) {
    const maskedValue = secret.value.substring(0, 8) + '...' + secret.value.substring(secret.value.length - 8)
