#!/usr/bin/env bun
/**
 * Update Brand Information from Excel Spreadsheet
 *
 * Reads brand information from the Excel pricelist and updates
 * product brand fields in the database.
 *
 * Usage:
 *   bun scripts/database/update-brands-from-excel.ts
 */
...
function readExcelFile(filePath: string): ProductRow[] {
  ...
  for (let rowIdx = 0; rowIdx < Math.min(10, jsonData.length); rowIdx++) {
    ...
    if (
      ...
      (rowText.includes('brand') || rowText.includes('category') || rowText.includes('description'))
    ) {
 ...
 */
async function main() {
  console.log('🚀 Updating Brand Information from Excel Spreadsheet\n');
  ...
  try {
    ...
    for (const product of products) {
      if (!product.brand) {
        console.log(`⚠️  ${product.sku}: No brand in spreadsheet, skipping`);
        skipped++;
        continue;
      }

      const result = await updateProductBrand(client, supplierId, product);
      
      if (result.updated) {
        console.log(`✅ ${product.sku}: Brand updated to "${product.brand}"`);
        updated++;
      } else {
