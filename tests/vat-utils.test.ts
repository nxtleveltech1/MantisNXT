...
import { 
  normalizePriceAndVat, 
  applyVatPolicyToRows
} from '@/lib/cmm/vat-utils'
...
describe('VAT Utils', () => {
  describe('normalizePriceAndVat', () => {
    it('should handle explicit EX-VAT price correctly', () => {
      ...
      expect(result.cost_price_ex_vat).toBe(100.00)
      expect(result.price_incl_vat).toBe(115.00)
    ...
    it('should handle ambiguous price data with warnings', () => {
      const row = {
        cost_price_ex_vat: 100.00,
        price_incl_vat: 120.00, // Inconsistent with 15% VAT
        vat_rate: 0.15,
        currency: 'ZAR'
      }
      ...
      expect(result.cost_price_ex_vat).toBe(100.00)
      expect(result.price_incl_vat).toBe(115.00) // Recalculated from EX-VAT
      expect(result.vat_rate).toBe(0.15)
      expect(result.price_source).toBe('explicit_ex_vat')
      expect(result.warnings).toContainEqual({
        field: 'price_incl_vat',
        message: 'Inconsistent INCL-VAT price (120.00 vs calculated 115.00), using EX-VAT as source'
      })
    })
    ...
    it('should use supplier-specific VAT policy', () => {
      ...
      expect(result.cost_price_ex_vat).toBe(100.00)
      expect(result.price_incl_vat).toBe(121.00)
    ...
    it('should handle zero VAT rate correctly', () => {
      ...
      expect(result.cost_price_ex_vat).toBe(100.00)
      expect(result.price_incl_vat).toBe(100.00)
  ...
  describe('applyVatPolicyToRows', () => {
    it('should process multiple rows with VAT normalization', async () => {
      ...
      expect(result.rows[1].cost_price_ex_vat).toBe(200.00)
    ...
    it('should handle supplier-specific VAT policy from database', async () => {
      ...
      const mockQuery = jest.fn().mockResolvedValue({
        rows: [{
          vat_rate: 0.20, // 20% for this supplier
          currency: 'EUR'
        }]
      })
      ...
      expect(result.rows[0].vat_rate).toBe(0.20)
