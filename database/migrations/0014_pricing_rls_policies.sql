...
CREATE POLICY "pricing_rule_delete" ON pricing_rule
    FOR DELETE USING (
        org_id = current_setting('app.current_org_id', true)::uuid AND
        (SELECT role FROM profile WHERE id = auth.uid()) = 'admin'
    );
...
CREATE POLICY "pricing_optimization_update" ON pricing_optimization
    FOR UPDATE USING (
        org_id = current_setting('app.current_org_id', true)::uuid AND
        (created_by = auth.uid() OR (SELECT role FROM profile WHERE id = auth.uid()) = 'admin')
    );
...
COMMENT ON POLICY "pricing_recommendation_update" ON pricing_recommendation IS 'Only admin and manager can approve/reject pricing recommendations';
COMMENT ON POLICY "competitor_pricing_write" ON competitor_pricing IS 'Managers can add competitor pricing data via scrapers';
...
VALUES ('0014_pricing_rls_policies')
