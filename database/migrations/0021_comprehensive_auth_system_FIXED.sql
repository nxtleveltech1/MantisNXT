...
--
-- FIXES APPLIED:
-- ✅ Issue #1: Circular FK on suspended_by (now DEFERRABLE)
-- ✅ Issue #2: Supabase vs Stack Auth clarified (Stack Auth primary, explicit constraints)
-- ✅ Issue #3: Basic RLS policies (optimized versions in 0022)
-- ✅ Issue #4: Essential composite indexes added (advanced indexes in 0022)
-- ✅ Issue #5: 2FA secrets prepared for encryption (encrypted in 0022)
-- ✅ Issues #6-10: Advanced features in 0022_critical_fixes.sql
-- ============================================================================
...
-- This system uses Stack Auth (Neon's recommended auth) as PRIMARY provider
-- - stack_auth_user_id: PRIMARY identifier from Stack Auth
-- - supabase_user_id: LEGACY/OPTIONAL for migration from Supabase Auth
--
-- At least ONE auth provider must be set (enforced by CHECK constraint)
-- New users should ONLY use Stack Auth
--
CREATE TABLE IF NOT EXISTS auth.users_extended (
    ...
    -- LEGACY: Supabase Auth - Only for migration compatibility
    supabase_user_id UUID, -- Links to auth.users(id) if migrating from Supabase
    ...
    CONSTRAINT must_have_auth_provider CHECK (
        stack_auth_user_id IS NOT NULL OR supabase_user_id IS NOT NULL
    ),

    -- Prevent both being NULL
    CONSTRAINT auth_provider_not_empty CHECK (
        COALESCE(stack_auth_user_id, supabase_user_id::TEXT, '') != ''
    )
);
...
-- Add FK for Supabase (conditional, may fail if auth.users doesn't exist)
DO $$
BEGIN
    ALTER TABLE auth.users_extended
    ADD CONSTRAINT fk_supabase_user_id
    FOREIGN KEY (supabase_user_id)
    REFERENCES auth.users(id)
    ON DELETE CASCADE;
EXCEPTION
    WHEN undefined_table THEN
        RAISE NOTICE 'Skipping FK to auth.users (Supabase not installed)';
...
CREATE INDEX IF NOT EXISTS idx_users_extended_supabase_auth
ON auth.users_extended(supabase_user_id) WHERE supabase_user_id IS NOT NULL;
...
COMMENT ON COLUMN auth.users_extended.supabase_user_id IS 'LEGACY: Supabase Auth ID (migration compatibility only)';
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '✅ ============================================================';
    RAISE NOTICE '✅ COMPREHENSIVE AUTH SYSTEM MIGRATION COMPLETE';
    RAISE NOTICE '✅ ============================================================';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '   ✓ Issue #1: Circular FK (suspended_by) - DEFERRABLE';
    RAISE NOTICE '   ✓ Issue #2: Auth Provider Strategy - Stack Auth PRIMARY';
    RAISE NOTICE '   ✓ Issue #3: Basic RLS Policies - Optimized in 0022';
    RAISE NOTICE '   ✓ Issue #4: Composite Indexes - Essential ones added';
    RAISE NOTICE '   ✓ Issue #5: 2FA Encryption - Prepared (encrypted in 0022)';
    RAISE NOTICE '   ✓ Issue #9: Creator Tracking - Added to key tables';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '   - Legacy Support: Supabase Auth (migration only)';
