...
-- Fixed: Removed NOW() from partial index
CREATE INDEX IF NOT EXISTS idx_uploads_expires ON spp.pricelist_uploads(expires_at);
...
-- Fixed: Changed created_at to queued_at
CREATE INDEX IF NOT EXISTS idx_jobs_org ON spp.extraction_jobs(org_id, queued_at DESC);
...
-- Fixed: Removed NOW() from partial index
CREATE INDEX IF NOT EXISTS idx_results_expires ON spp.extraction_results(expires_at);