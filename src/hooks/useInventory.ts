...
export function useInventory(options: UseInventoryOptions = {}) {
  ...
  const fetchInventory = useCallback(async (searchFilters?: InventorySearchFilters) => {
    setLoading(true)
    setError(null)
    ...
    try {
      ...
      if (filtersToUse?.category?.length) params.append('category', filtersToUse.category.join(','))
      if (filtersToUse?.status?.length) params.append('status', filtersToUse.status.join(','))
      if (filtersToUse?.warehouse?.length) params.append('warehouse', filtersToUse.warehouse.join(','))
      if (filtersToUse?.supplier?.length) params.append('supplier', filtersToUse.supplier.join(','))
      if (filtersToUse?.lowStock !== undefined) params.append('lowStock', filtersToUse.lowStock.toString())
      if (filtersToUse?.outOfStock !== undefined) params.append('outOfStock', filtersToUse.outOfStock.toString())
