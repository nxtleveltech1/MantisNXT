...
export function usePurchaseOrders(options: UsePurchaseOrdersOptions = {}) {
  ...
  const fetchPurchaseOrders = useCallback(async (searchFilters?: PurchaseOrderSearchFilters) => {
    ...
    try {
      ...
      if (filtersToUse?.supplier_id?.length) params.append('supplier_id', filtersToUse.supplier_id.join(','))
      if (filtersToUse?.min_amount !== undefined) params.append('min_amount', filtersToUse.min_amount.toString())
      if (filtersToUse?.max_amount !== undefined) params.append('max_amount', filtersToUse.max_amount.toString())
      if (filtersToUse?.page !== undefined) params.append('page', filtersToUse.page.toString())
      if (filtersToUse?.limit !== undefined) params.append('limit', filtersToUse.limit.toString())
...
export function usePurchaseOrderMetrics() {
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch purchase order metrics'
      setError(errorMessage)
