...
export function useProgressTracker(
  ...
  const calculateOverallProgress = useCallback(
    (stages: ProgressStage[], currentIndex: number, currentStageProgress: number = 0): number => {
      let totalWeight = 0;
  ...
  const calculateTimeEstimates = useCallback(
    (
      currentProgress: number
    ): { timeRemaining?: number; estimatedCompletion?: Date; throughput?: number } => {
  ...
  const updateProgress = useCallback(
    (update: ProgressUpdate) => {
      setTracker(prev => {
        ...
        const errors = newStages
          .filter(s => s.status === 'error')
          .map(s => s.error || 'Unknown error');
        const warnings = newStages
          .filter(s => s.status === 'warning')
          .map(s => s.error || 'Warning');
  ...
  const errorCurrentStage = useCallback(
    ...
    getStageProgress: (stageId: string) => {
      ...
      switch (stage.status) {
        case 'completed':
          return 100;
        case 'running':
          return 50; // Assume halfway if no specific progress
        case 'error':
          return 0;
        default:
          return 0;
