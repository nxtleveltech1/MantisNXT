...
export class WebSearchService {
  ...
  async searchSuppliers(query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
    try {
      ...
      console.log(`🔍 WebSearchService: Starting AI-powered search for: "${query}" with maxResults ${maxResults}`)
      ...
      console.log('🔍 AI Results preview:', aiResults.slice(0, 2).map(r => ({ title: r.title, source: r.source, url: r.url, extractedData: !!r.extractedData })))

      return aiResults

    } catch (error) {
      console.error('❌ WebSearchService: AI search error:', error)
      throw new Error(`AI-powered web search failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
   ...
   */
  private async performAISearch(query: string, maxResults: number, options: SearchOptions): Promise<SearchResult[]> {
    // Initialize AI-powered scraping service
    const aiScrapingService = new (require('./AIPoweredWebScrapingService').AIPoweredWebScrapingService)()
    ...
    for (const urlInfo of targetUrls.slice(0, maxResults)) {
      try {
        ...
        if (scrapedData) {
          ...
          console.log(`✅ AI extracted data for: ${scrapedData.metadata.companyName || urlInfo.domain}`)
   ...
   */
  private generateTargetUrlsFromQuery(query: string): Array<{url: string, source: string, domain: string}> {
    const lowerQuery = query.toLowerCase()
    const urls: Array<{url: string, source: string, domain: string}> = []
    ...
    const industryUrls = {
      ...
      'music': [
        ...
        { url: 'https://www.musicbusinessworldwide.com', source: 'musicbusinessworldwide.com', domain: 'musicbusinessworldwide.com' }
      ],
      'healthcare': [
        { url: 'https://www.healthcareitnews.com', source: 'healthcareitnews.com', domain: 'healthcareitnews.com' },
        { url: 'https://www.fiercehealthcare.com', source: 'fiercehealthcare.com', domain: 'fiercehealthcare.com' }
      ],
      'manufacturing': [
        { url: 'https://www.manufacturing.net', source: 'manufacturing.net', domain: 'manufacturing.net' },
        { url: 'https://www.industryweek.com', source: 'industryweek.com', domain: 'industryweek.com' }
      ]
    }

    // Generate URLs based on detected industry
    const detectedIndustry = this.detectPrimaryIndustry(lowerQuery)
    if (industryUrls[detectedIndustry]) {
      urls.push(...industryUrls[detectedIndustry])
    }

    // Add general business URLs
    urls.push(
      { url: 'https://www.businessinsider.com', source: 'businessinsider.com', domain: 'businessinsider.com' },
  ...
  private convertAIToSearchResult(scrapedData: any, source: string): SearchResult {
    ...
    return {
      ...
      description: scrapedData.description || scrapedData.metadata.businessDescription || `Professional ${industry.toLowerCase()} services provider with comprehensive expertise.`,
      url: scrapedData.url,
      source: source,
   ...
   */
  private detectPrimaryIndustry(query: string): string {
  ...
  private generateMockResults(query: string, maxResults: number): SearchResult[] {
    console.log(`🌐 WebSearchService: generateMockResults called with query="${query}", maxResults=${maxResults}`)
  ...
  private analyzeQueryIntent(query: string): {
    ...
    const industryPatterns = {
      'Technology': ['tech', 'software', 'it', 'computing', 'digital', 'cloud', 'saas', 'app', 'platform', 'api', 'development', 'programming'],
      'Music & Entertainment': ['music', 'entertainment', 'audio', 'record', 'label', 'artist', 'sound', 'distribution', 'studio', 'production'],
      'Manufacturing': ['manufacturing', 'production', 'factory', 'industrial', 'components', 'parts', 'assembly', 'machinery', 'equipment'],
      'Healthcare': ['healthcare', 'medical', 'hospital', 'pharmaceutical', 'health', 'medicine', 'clinic', 'pharmacy', 'dental'],
      'Finance': ['finance', 'banking', 'financial', 'investment', 'insurance', 'accounting', 'consulting', 'capital'],
      'Education': ['education', 'school', 'university', 'training', 'learning', 'academy', 'course', 'teaching'],
      'Construction': ['construction', 'building', 'engineering', 'contractor', 'architect', 'design', 'civil', 'structural'],
      'Retail': ['retail', 'shop', 'store', 'commerce', 'sales', 'consumer', 'wholesale', 'distribution'],
      'Transportation': ['transportation', 'logistics', 'shipping', 'freight', 'delivery', 'courier', 'transport'],
      'Energy': ['energy', 'renewable', 'solar', 'wind', 'power', 'utility', 'oil', 'gas', 'electrical']
    }
   ...
   */
  private generateIntelligentResults(query: string, analysis: any, maxResults: number): SearchResult[] {
    const results: SearchResult[] = []
      ...
      'Technology': [
        {
          ...
          description: 'Leading software development company specializing in enterprise solutions, cloud migration, and digital transformation. Certified AWS and Microsoft partners with 15+ years experience.',
          url: 'https://www.techflow-solutions.com',
          domain: 'techflow-solutions.com'
        },
        {
          title: 'CloudFirst Technologies - Cloud Infrastructure',
          description: 'Enterprise cloud solutions provider offering migration, management, and optimization services. Expert in AWS, Azure, and Google Cloud platforms.',
          url: 'https://www.cloudfirst-tech.com',
          domain: 'cloudfirst-tech.com'
        },
        {
          title: 'InnovateTech SA - Digital Innovation',
          description: 'Full-service technology company providing custom software, mobile apps, and IT consulting. Specializing in fintech and e-commerce solutions.',
      ...
      'Music & Entertainment': [
        {
          ...
          description: 'Premier music distribution company offering digital streaming, physical CD/vinyl distribution, and artist services. Trusted by 500+ artists across Africa.',
        ...
        },
        {
          ...
          description: 'Full-service audio production company providing recording, mixing, mastering, and worldwide distribution services for artists and labels.',
      ...
      'Manufacturing': [
        {
          ...
          description: 'ISO 9001 certified manufacturer of precision components for automotive, aerospace, and electronics industries. 25+ years of manufacturing excellence.',
        ...
        },
        {
          ...
          description: 'International manufacturing company specializing in heavy machinery, industrial equipment, and custom manufacturing solutions for global markets.',
      ...
      'Healthcare': [
        {
          ...
          description: 'Leading supplier of medical equipment, diagnostic tools, and healthcare technology solutions. Certified medical device distributor with nationwide coverage.',
      ...
      'Finance': [
        {
          ...
          description: 'Comprehensive financial services including investment planning, insurance solutions, and wealth management. Licensed financial advisors with 20+ years experience.',
   ...
   */
  private getLocationData(location: string, industry: string): any[] {
