 ...
 */
export async function getSupplierProfile(supplierId: string, profileName: string = 'default'): Promise<SupplierProfile | null> {
  try {
    const response = await fetch(`/api/supplier-profiles?supplier_id=${supplierId}&profile_name=${profileName}`)
    if (!response.ok) {
 ...
 * Get supplier rules
 */
export async function getSupplierRulesClient(supplierId: string, triggerEvent: string = 'pricelist_upload'): Promise<SupplierRule[]> {
  try {
    const response = await fetch(`/api/suppliers/${supplierId}/rules?trigger_event=${triggerEvent}`)
