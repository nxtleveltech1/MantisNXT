...
export class ApiMiddleware {
    ...
    return async (request: NextRequest): Promise<NextResponse> => {
      ...
      try {
        ...
        if (user && options.requiredRole && user.role !== options.requiredRole) {
          return this.createErrorResponse(`Role '${options.requiredRole}' required`, 403, {
      ...
      return this.withAuth(
        async (request: NextRequest, context: RequestContext) => {
          try {
            ...
            for (const [key, value] of formData.entries()) {
              if (value instanceof File) {
                ...
                if (options.allowedTypes && !options.allowedTypes.includes(value.type)) {
                  return this.createErrorResponse(`File type ${value.type} not allowed`, 400, {
            ...
            if (files.length === 0) {
              return this.createErrorResponse('No files uploaded', 400);
            }

            return await handler(request, context, files);
          } catch (error) {
            console.error('File upload error:', error);
            return this.createErrorResponse('File upload failed', 400, {
              error: error instanceof Error ? error.message : 'Unknown error',
            });
          }
        },
  ...
  private static getClientIP(request: NextRequest): string {
    ...
    return (
      cfConnectingIP || xForwardedFor?.split(',')[0]?.trim() || xRealIP || request.ip || '127.0.0.1'
    );
  }
   ...
   */
  static createErrorResponse(message: string, status: number, details?: unknown): NextResponse {
