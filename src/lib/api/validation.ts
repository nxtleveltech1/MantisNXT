...
export const EnhancedInventoryItemSchema = z.object({
  ...
  maxStock: z.number()
    .min(0, 'Max stock must be non-negative'),
  minStock: z.number()
...
export const InventorySearchSchema = PaginationSchema.extend({
  ...
  sortBy: z.enum([
    'name', 'sku', 'category', 'currentStock', 'value',
    'lastStockUpdate', 'status', 'createdAt', 'updatedAt'
  ]).optional()
})
...
export const EnhancedSupplierSchema = z.object({
  ...
  paymentTerms: z.object({
    ...
    discountDays: z.number()
      .min(0, 'Discount days must be non-negative')
      .optional(),
    currency: z.string()
      .length(3, 'Currency must be 3 characters')
      .regex(/^[A-Z]{3}$/, 'Currency must be uppercase')
...
export const SupplierSearchSchema = PaginationSchema.extend({
  ...
  sortBy: z.enum([
    'name', 'code', 'performance', 'leadTime', 'lastOrder',
    'rating', 'totalValue', 'createdAt', 'updatedAt'
  ]).optional()
})
...
export const StockMovementSchema = z.object({
  ...
  quantity: z.number()
    .min(-999999999, 'Quantity too small')
    .max(999999999, 'Quantity too large'),
  unitCost: z.number().min(0).optional(),
  unitPrice: z.number().min(0).optional(),
  fromLocationId: z.string().optional(),
  toLocationId: z.string().optional(),
  referenceType: z.enum(['purchase_order', 'sales_order', 'transfer_order', 'adjustment', 'inventory_count']).optional(),
...
export const AnalyticsQuerySchema = z.object({
  metrics: z.array(z.enum([
    'inventory_value', 'stock_levels', 'turnover_rate', 'supplier_performance',
    'category_distribution', 'movement_trends', 'cost_analysis', 'demand_forecast'
  ])),
  dimensions: z.array(z.enum([
    'time', 'category', 'supplier', 'warehouse', 'location', 'product', 'brand'
  ])).optional(),
  filters: z.object({
