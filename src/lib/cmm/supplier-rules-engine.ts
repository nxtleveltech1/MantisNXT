...
export interface SupplierRule {
  ...
  supplierId: string;
  ruleName: string;
  ruleType: 'validation' | 'transformation' | 'approval' | 'notification' | 'enforcement';
  triggerEvent: string;
...
export async function applyPricelistRulesToExcel(
  ...
  const transform = rules.find(
    r => r.ruleType === 'transformation' && (r.ruleConfig as any)?.join_sheets
  );
  if (!transform) return [];
  ...
  function fuzzyFind(nameLike: string): string | null {
    const thresh =
      typeof cfg.sheet_matcher?.threshold === 'number' ? cfg.sheet_matcher.threshold : 0.6;
    const target = norm(nameLike);
 ...
 */
export async function updateSupplierProfile(profile: SupplierProfile): Promise<void> {
  await query(
    `
...
function executeRule(
  ...
  try {
    ...
    result.passed = false;
    result.errorMessage = error instanceof Error ? error.message : 'Rule execution failed';
    if (rule.isBlocking) {
      result.blocked = true;
    }
  }
...
function executeValidationRule(
  ...
  if (config.required && (value === undefined || value === null || value === '')) {
    result.errorMessage = rule.errorMessageTemplate || `Field ${field} is required`;
  ...
  if (config.pattern && value) {
    ...
    if (!regex.test(String(value))) {
      result.errorMessage =
        rule.errorMessageTemplate || `Field ${field} does not match required pattern`;
  ...
  if (config.min !== undefined || config.max !== undefined) {
    ...
    if (!isNaN(numValue)) {
      if (config.min !== undefined && numValue < config.min) {
        result.errorMessage =
          rule.errorMessageTemplate || `Field ${field} must be at least ${config.min}`;
        result.executionResult = { field, min: config.min, value: numValue };
        return false;
      }
      if (config.max !== undefined && numValue > config.max) {
        result.errorMessage =
          rule.errorMessageTemplate || `Field ${field} must be at most ${config.max}`;
  ...
  if (config.max_variance_percent && config.compare_field) {
    ...
    if (!isNaN(price) && !isNaN(cost) && cost > 0) {
      ...
      if (Math.abs(variance) > config.max_variance_percent) {
        result.errorMessage =
          rule.errorMessageTemplate ||
          `Price variance ${variance.toFixed(2)}% exceeds maximum ${config.max_variance_percent}%`;
        result.executionResult = {
          field,
          variance,
