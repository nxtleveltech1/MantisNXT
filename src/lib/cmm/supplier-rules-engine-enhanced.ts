...
async function processJoinSheets(
  ...
  if (!leftSheet || !rightSheet) {
    throw new Error(
      `Could not find sheets for join: ${config.left_sheet} or ${config.right_sheet}`
    );
  }
...
async function processSheetLoop(
  ...
  for (const sheetName of wb.SheetNames) {
    const shouldInclude =
      config.include?.some((pattern: string) => {
        if (pattern === '*') return true;
        return norm(sheetName).includes(norm(pattern));
      }) ?? true;

    const shouldExclude =
      config.exclude?.some((pattern: string) => {
        return norm(sheetName).includes(norm(pattern));
      }) ?? false;
  ...
  for (const sheetName of sheetsToProcess) {
    ...
    const brandName =
      config.brand_source === 'sheet_name' ? extractBrandFromSheetName(sheetName) : undefined;
 ...
 */
function extractCategoryFromSectionHeader(
  sheetName: string,
  rowIndex: number,
  allRows: unknown[][]
): string | undefined {
