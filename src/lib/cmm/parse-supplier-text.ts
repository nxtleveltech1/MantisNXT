...
export function parseSupplierText(text: string, supplierHint?: string): ParseResult {
  const lines = text
    .trim()
    .split('\n')
  ...
  lines.forEach((line, index) => {
    try {
      ...
      if (trimmedFields.length < 2) {
        errors.push(`Line ${index + 1}: Insufficient fields (minimum 2 required)`);
        return;
      }
      ...
      for (let i = 2; i < trimmedFields.length; i++) {
        ...
        if (priceMatch && !price) {
          const numericValue = Number.parseFloat(priceMatch[0].replace(/,/g, ''));
        ...
        if (stockTypes.some(type => field.toLowerCase().includes(type))) {
          stockType = field.toLowerCase().includes('stock') ? 'stock' : field.toLowerCase();
          continue;
        }
        ...
        if (price && !cost && priceMatch) {
          const numericValue = Number.parseFloat(priceMatch[0].replace(/,/g, ''));
          if (!isNaN(numericValue) && numericValue > 0 && numericValue < price) {
            cost = numericValue;
          }
        }
      }
      ...
      if (!brandColumnValue || !seriesRange) {
        ...
        for (const commonBrand of commonBrands) {
          if (descriptionLower.includes(commonBrand)) {
            if (!brandColumnValue)
              brandColumnValue = commonBrand.charAt(0).toUpperCase() + commonBrand.slice(1);
          }
        }
      ...
      if (trimmedFields.length >= 4) {
        ...
        if (!brandColumnValue && trimmedFields[2] && !pricePattern.test(trimmedFields[2])) {
          brandColumnValue = trimmedFields[2];
        }
        if (!seriesRange && trimmedFields[3] && !pricePattern.test(trimmedFields[3])) {
      ...
      if (!sku || !description) {
        errors.push(`Line ${index + 1}: Missing required fields (SKU and description)`);
        return;
      }
