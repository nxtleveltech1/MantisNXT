...
export async function getProducts({ uncategorized = false }: { uncategorized?: boolean } = {}) {
...
export async function upsertProductAttributes(
  ...
  if (p.supplierId) {
    ...
    const cost =
      (p.attributes as unknown)?.cost || (p.attributes as unknown)?.dealerPrice || undefined;
    ...
    const ruleResult = await executeSupplierRules(p.supplierId, 'upsert', ruleData, context);
  ...
  if (existing.length === 0) {
    ...
    if (p.supplierId && p.supplierId !== exists.supplierId) {
      ...
      return {
        ...
        conflict: {
          sku,
          type: 'duplicate',
          message: 'SKU already exists from a different supplier. New SKU required.',
        },
      };
    }
    ...
    const compare = (
      ...
      if (attempted === undefined || attempted === null || attempted === '') return;
...
export async function assignCategory(sku: string, categoryId: string) {
  ...
  await query(`update products set category_id = $1, updated_at = now() where sku = $2`, [
    categoryId,
    sku,
  ]);
}
...
export async function applyRules() {
  ...
  for (const p of products) {
    for (const r of rules) {
      if ((p.description || '').toLowerCase().includes(r.keyword.toLowerCase())) {
...
export async function recordUpload(params: {
  ...
  const inserted = params.results.filter(r => r.outcome === 'inserted').length;
  const updated = params.results.filter(r => r.outcome === 'updated').length;
  const conflictCount = params.results.filter(r => r.outcome === 'conflict').length;
