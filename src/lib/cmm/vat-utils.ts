...
export function normalizePriceAndVat(
  ...
  const priceExVat = extractNumericValue(row, [
    'cost_price_ex_vat',
    'price_ex_vat',
    'nett_excl',
    'dealer_ex',
  ...
  const priceIncVat = extractNumericValue(row, [
    'price_incl_vat',
    'price_inc_vat',
    'retail_incl',
    'rsp_incl',
...
function detectPriceColumns(
  ...
  for (const [key, value] of Object.entries(row)) {
    ...
    // Check for INCL VAT indicators
      ...
      keywords.some(k => lowerKey.includes(k.toLowerCase()) && k.toLowerCase().includes('inc'))
    ) {
      result.inc = numValue;
    }
  }
...
export function applyVatPolicyToRows(
  ...
  const vatPolicy: VatPolicy =
    typeof vatPolicyOrSupplierId === 'string'
      ? {
          rate: 0.15,
          mode: 'detect',
          detect_keywords: ['ex', 'inc', 'excl', 'incl', 'nett', 'retail'],
        }
