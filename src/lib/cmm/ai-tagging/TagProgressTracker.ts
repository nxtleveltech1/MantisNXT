 ...
 * Manages real-time progress tracking for AI tagging jobs
 * Mirrors: src/lib/cmm/ai-categorization/ProgressTracker.ts
 */
...
export class TagProgressTracker {
  ...
  async completeBatchProgress(params: {
    ...
    const sql = `
      UPDATE core.ai_tagging_progress
      SET 
        successful_count = $2,
        failed_count = $3,
        skipped_count = $4,
  ...
  async updateJobProgress(params: {
    ...
    await dbQuery(sql, [
      ...
      params.successful_taggings,
      params.failed_taggings,
  ...
  async cleanupOldProgress(daysToKeep: number = 30): Promise<number> {
    const sql = `
      DELETE FROM core.ai_tagging_progress
