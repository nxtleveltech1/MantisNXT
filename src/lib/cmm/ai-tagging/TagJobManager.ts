/**
 * Tag Job Manager Service
 * Manages AI tagging jobs with chunked processing and resumability
 * Mirrors: src/lib/cmm/ai-categorization/JobManager.ts
 */
...
export class TagJobManager {
  ...
  private progressTracker: TagProgressTracker;
  ...
  async createJob(params: JobParams): Promise<string> {
    ...
    const sql = `
      INSERT INTO core.ai_tagging_job (
        job_type,
        total_products,
        batch_size,
        filters,
  ...
  async processJob(jobId: string): Promise<void> {
    ...
    console.log(`[TagJobManager] Job ${jobId} marked as active`);

    try {
      // Get job details
      console.log(`[TagJobManager] Fetching job details for ${jobId}`);
      ...
      // Mark as running
      console.log(`[TagJobManager] Updating job status to 'running'`);
      await this.updateJobStatus(jobId, 'running');
      await this.updateJobStartTime(jobId);

      const config = job.config || {};
      const filters = job.filters || {};
      console.log(`[TagJobManager] Job config:`, config);
      console.log(`[TagJobManager] Job filters:`, filters);
      ...
      while (currentOffset < job.total_products) {
        ...
        if (currentStatus === 'cancelled' || currentStatus === 'paused') {
          console.log(`[TagJobManager] Job ${jobId} ${currentStatus}, stopping processing`);
          break;
        }

        try {
          console.log(
            `[TagJobManager] Job ${jobId} fetching batch ${batchNumber} (offset=${currentOffset}, size=${job.batch_size})`
          );
          ...
          if (products.length === 0) {
            console.log(`[TagJobManager] No more products to process for job ${jobId}`);
            break;
          }
          ...
          console.log(
            `[TagJobManager] Job ${jobId} batch ${batchNumber} progress record ${progressId} created`
          );

          // Mark products as processing
          await this.taggingEngine.markProductsAsProcessing(
            products.map(p => p.supplier_product_id)
          );
          console.log(
            `[TagJobManager] Job ${jobId} batch ${batchNumber} marked ${products.length} products as processing`
          );
          ...
          console.log(
            `[TagJobManager] Job ${jobId} batch ${batchNumber} tagBatch returned: success=${batchResult.successful}, failed=${batchResult.failed}, skipped=${batchResult.skipped}, pending_review=${batchResult.pending_review}`
          );

          // Apply results to database
          const applyResult = await this.taggingEngine.applyTaggingResults(
            batchResult.results,
            jobId
          );
          console.log(
            `[TagJobManager] Job ${jobId} batch ${batchNumber} apply results -> errors=${applyResult.errors.length}`
          );
          ...
          console.log(
            `[TagJobManager] Job ${jobId} batch ${batchNumber}: ${batchResult.successful} succeeded, ${batchResult.failed} failed, ${batchResult.skipped} skipped, ${batchResult.pending_review} pending_review`
          );
          ...
          console.error(
            `[TagJobManager] ERROR processing batch ${batchNumber} for job ${jobId}:`,
            error
          );
          ...
          if (job_updated && job_updated.error_count >= maxRetries) {
            console.error(`[TagJobManager] Job ${jobId} exceeded max retries, failing`);
      ...
      if (finalStatus === 'running') {
        ...
        console.log(`[TagJobManager] Job ${jobId} completed successfully`);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Job processing failed';
      console.error(`[TagJobManager] Fatal error processing job ${jobId}:`, error);
      await this.updateJobStatus(jobId, 'failed', errorMessage);
    } finally {
  ...
  async resumeJob(jobId: string): Promise<void> {
    ...
    this.processJob(jobId).catch(err => {
      console.error(`[TagJobManager] Error resuming job ${jobId}:`, err);
    });
  }
  ...
  async getJob(jobId: string): Promise<Job | null> {
    ...
    return {
      ...
      successful_taggings: Number(row.successful_taggings),
      failed_taggings: Number(row.failed_taggings),
  ...
  private async fetchProductsForBatch(
    ...
    console.log(
      `[TagJobManager] fetchProductsForBatch offset=${offset} batchSize=${batchSize} -> ids=${productIds.length}`
    );
  ...
  private async getProductIdsForBatch(
    ...
    console.log(
      `[TagJobManager] getProductIdsForBatch offset=${offset} batchSize=${batchSize} filters=${JSON.stringify(
        filters
      )} -> ${ids.length} ids`
    );
    return ids;
  ...
  private async estimateProducts(filters: JobFilters): Promise<number> {
    ...
    if (filters.status && filters.status.length > 0) {
      whereClauses.push(`ai_tagging_status = ANY($${paramCounter}::varchar[])`);
      queryParams.push(filters.status);
      paramCounter++;
    }

    if (filters.exclude_tagged) {
      whereClauses.push(`supplier_product_id NOT IN (
        SELECT DISTINCT supplier_product_id FROM core.ai_tag_assignment
      )`);
    }
