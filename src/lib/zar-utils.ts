...
export function getStatusColor(status: string): string {
  const statusColors: Record<string, string> = {
    ...
    exceptions: "bg-yellow-100 text-yellow-800 border-yellow-200",
...
// South African specific utility functions
...
export function getBEELevelColor(level: number): string {
...
export function calculateVAT(amount: number): number {
  return amount * 0.15 // 15% VAT rate in South Africa
}
...
export function removeVAT(amountIncVAT: number): number {
