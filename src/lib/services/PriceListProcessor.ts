...
export class PriceListProcessor {
  ...
  async processFile(config: unknown): Promise<ProcessingResult> {
    ...
    try {
      ...
      const summary = await this.generateSummary(parsedData, validatedData, importResult);
      this.updateSession(sessionId, { status: 'completed', progress: 100 });

      const executionTime = Date.now() - startTime;

      console.log(
        `✅ Price list processing completed - Session: ${sessionId} (${executionTime}ms)`
      );
      ...
      throw new Error(
        `Price list processing failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async detectFieldMappings(headers: string[]): Promise<Record<string, string>> {
    ...
    console.log(
      `🎯 Field mapping confidence: ${Object.keys(mappings).length}/${Object.keys(fieldPatterns).length} fields mapped`
    );
  ...
  private async validateAndTransformData(
    ...
    console.log(
      `✅ Data validation completed: ${stats.validRows} valid, ${stats.invalidRows} invalid`
    );
  ...
  private async handleDuplicates(
    ...
    console.log(
      `🔀 Duplicate handling: ${processedEntries.length} to process, ${skippedEntries.length} skipped`
    );
  ...
  private async bulkImport(
    ...
    console.log(
      `✅ Bulk import completed: ${result.created} created, ${result.updated} updated, ${result.skipped} skipped`
    );
  ...
  private mergeEntries(entries: unknown[]): unknown {
    ...
    for (let i = 1; i < entries.length; i++) {
      ...
      if (
        entry.description &&
        (!merged.description || entry.description.length > merged.description.length)
      ) {
  ...
  private async createBackup(supplierId: string, entries: unknown[]): Promise<string> {
    ...
    await this.pool.query(query, [backupId, supplierId, JSON.stringify(entries), entries.length]);
