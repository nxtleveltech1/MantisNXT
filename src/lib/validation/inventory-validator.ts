...
export const InventoryItemValidationSchema = z.object({
  ...
  brand: z.string().max(200, 'Brand name cannot exceed 200 characters').optional(),
  ...
  dimensions_cm: z
    ...
    .regex(
      /^\d+(\.\d+)?x\d+(\.\d+)?x\d+(\.\d+)?$/,
      'Dimensions must be in format "LxWxH" (e.g., "10.5x20x30")'
    )
    .optional(),
...
export const SupplierProductValidationSchema = z.object({
  supplier_sku: z.string().max(100, 'Supplier SKU cannot exceed 100 characters').optional(),
...
export class InventoryValidator {
  ...
  public static async validateInventoryItem(
    ...
    try {
      ...
      transformedData = await this.applyDataTransformations(
        transformedData,
  ...
  private static async validateReferences(
    ...
    if (data.category) {
      ...
      const categoryExists = categories.some(
        cat => cat.name.toLowerCase() === data.category.toLowerCase() && cat.is_active
      );
    ...
    if (data.brand) {
      ...
      const brandExists = brands.some(
        brand => brand.name.toLowerCase() === data.brand.toLowerCase() && brand.is_active
      );
  ...
  // Cross-field validations
  private static performCrossFieldValidations(data: unknown, issues: ValidationIssue[]): void {
    ...
    if (data.dimensions_cm) {
      ...
      if (dimensions.length === 3 && dimensions.every((d: number) => !isNaN(d))) {
        ...
        if (dimensions.some((d: number) => d > 10000)) {
          // 100 meters
          issues.push({
            field: 'dimensions_cm',
        ...
        if (dimensions.some((d: number) => d < 0.1)) {
          // 1mm
          issues.push({
            field: 'dimensions_cm',
            value: data.dimensions_cm,
  ...
  public static getValidationSummary(issues: ValidationIssue[]): {
    ...
    const recommendations = issues.filter(i => i.suggestion).map(i => i.suggestion!);
