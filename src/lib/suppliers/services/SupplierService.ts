...
export class SupplierService {
  ...
  async createManySuppliers(data: CreateSupplierData[]): Promise<Supplier[]> {
    ...
    for (const supplierData of data) {
      ...
      if (!validation.isValid) {
        throw new Error(
          `Validation failed for supplier ${supplierData.name}: ${validation.errors.map(e => e.message).join(', ')}`
        );
      }
    }
  ...
  async updateManySuppliers(
    updates: Array<{ id: string; data: UpdateSupplierData }>
  ): Promise<Supplier[]> {
    ...
    for (const update of updates) {
      ...
      if (!validation.isValid) {
        throw new Error(
          `Validation failed for supplier ${update.id}: ${validation.errors.map(e => e.message).join(', ')}`
        );
      }
    }
