...
export class AISupplierDiscoveryService {
  ...
  async discoverSuppliers(request: AISupplierDiscovery): Promise<AISupplierMatch[]> {
    ...
    for (let index = 0; index < maxResults; index++) {
      ...
      const confidence = Math.min(0.95, Math.max(minConfidence, 0.85 - index * 0.05));
  ...
  async enrichSupplierData(
    ...
    return {
      ...
      additionalInfo: {
        ...
        riskAssessment:
          'Low operational risk; medium supply chain risk due to regional dependencies.',
  ...
  private buildReasoning(request: AISupplierDiscovery): string[] {
    const reasoning: string[] = [`Matches search intent for "${request.query}".`];
