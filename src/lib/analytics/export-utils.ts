...
export class AnalyticsExporter {
  ...
  static exportSavingsReport(savingsData: any[]): void {
    ...
    const rows = savingsData.map(item => {
      const totalSavings = item.negotiated + item.processImprovement + item.volumeDiscount + item.alternativeSource
  ...
  static exportKPIDashboard(kpiData: any): void {
    ...
    const rows = [
      ...
      ['Average Savings', this.formatPercent(kpiData.avgSavings), this.formatPercent(kpiData.avgSavingsChange), kpiData.avgSavingsChange > 0 ? 'Improved' : 'Declined'],
      ['Supplier Count', kpiData.supplierCount.toString(), this.formatPercent(kpiData.supplierCountChange), kpiData.supplierCountChange > 0 ? 'Increased' : 'Decreased'],
      ['On-Time Delivery', this.formatPercent(kpiData.onTimeDelivery), this.formatPercent(kpiData.onTimeDeliveryChange), kpiData.onTimeDeliveryChange > 0 ? 'Improved' : 'Declined'],
      ['Quality Score', this.formatPercent(kpiData.qualityScore), this.formatPercent(kpiData.qualityScoreChange), kpiData.qualityScoreChange > 0 ? 'Improved' : 'Declined'],
      ['Cost Variance', this.formatPercent(kpiData.costVariance), this.formatPercent(kpiData.costVarianceChange), kpiData.costVariance < 0 ? 'Under Budget' : 'Over Budget'],
      ['Risk Score', kpiData.riskScore.toString(), this.formatPercent(kpiData.riskScoreChange), kpiData.riskScoreChange < 0 ? 'Improved' : 'Worsened'],
      ['Contract Compliance', this.formatPercent(kpiData.contractCompliance), this.formatPercent(kpiData.contractComplianceChange), kpiData.contractComplianceChange > 0 ? 'Improved' : 'Declined']
    ]
