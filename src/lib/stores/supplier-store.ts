...
export const useSupplierStore = create<SupplierStore>()(
  devtools(
    (set, get) => ({
      ...
      fetchSuppliers: async () => {
        ...
        try {
          ...
          if (filters.performance_tier?.length)
            params.append('performance_tier', filters.performance_tier.join(','));
      ...
      addSupplier: async (supplierData: SupplierFormData) => {
        ...
        try {
          ...
          const data = await response.json();
          set((state: SupplierStore) => ({
            suppliers: [...state.suppliers, data.data as Supplier],
            loading: false,
          }));
        ...
        try {
          ...
          set((state: SupplierStore) => ({
            suppliers: state.suppliers.map((s: Supplier) =>
              s.id === id ? (data.data as Supplier) : s
            ),
            loading: false,
          }));
    ...
    {
      name: 'supplier-store',
      partialize: (state: SupplierStore) => ({
