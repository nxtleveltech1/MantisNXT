 ...
 */
export function groupSohBySupplier(sohData: SOHBySupplier[]): Map<
  string,
  {
...
export function calculatePriceChangeStats(products: ProductTableBySupplier[]): {
  ...
  const average_change_percent =
    changesOnly.length > 0
      ? changesOnly.reduce((sum, p) => sum + (p.price_change_percent || 0), 0) / changesOnly.length
      : 0;

  const max_increase =
    increases.length > 0 ? Math.max(...increases.map(p => p.price_change_percent!)) : 0;

  const max_decrease =
    decreases.length > 0 ? Math.min(...decreases.map(p => p.price_change_percent!)) : 0;
...
export async function completeISIWorkflow(
  ...
  try {
    ...
    const createResult = await api.createSelection(selectionName, options.description, createdBy);
    ...
    // 2. Select products
    options.on_progress?.('selecting', {
      selection_id: selectionId,
      count: supplierProductIds.length,
    });
...
export async function batchSelectProducts(
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      totalFailed += chunk.length;
      errors.push(
        `Chunk ${i + 1} error: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
...
export function validatePriceChange(
  ...
  const severity: 'major' | 'moderate' | 'minor' =
    Math.abs(changePercent) > 20 ? 'major' : Math.abs(changePercent) > 10 ? 'moderate' : 'minor';
