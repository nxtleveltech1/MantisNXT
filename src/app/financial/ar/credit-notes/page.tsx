/**
 * Accounts Receivable - Credit Notes Page
 */
...
export default function ARCreditNotesPage() {
  ...
  useEffect(() => {
    async function fetchCreditNotes() {
      ...
      try {
        if (source === 'xero') {
          const response = await fetch(buildClientXeroUrl('/api/xero/sync/credit-notes?type=ACCRECCREDIT'), {
          ...
          const response = await fetch(`/api/v1/financial/ar/credit-notes?org_id=${orgId}`);
          const result = await response.json();
    ...
    >
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-muted-foreground">Manage accounts receivable credit notes</p>
          </div>
          <div className="flex items-center gap-2">
            <Label htmlFor="ar-cn-source" className="text-sm text-muted-foreground">
              Source
            </Label>
            <Select value={source} onValueChange={(v) => setSource(v as ListSource)}>
              <SelectTrigger id="ar-cn-source" className="w-[120px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>List of customer credit notes</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
