...
import { FileUp, Settings, FileCog, TrendingUp, History, CheckCircle2 } from 'lucide-react'

type Supplier = { id: string; name: string; code?: string; status?: string; tags?: string[] }
...
export default function SupplierProfilePage() {
  ...
  useEffect(() => {
    const loadSupplier = async () => {
      try {
        ...
        if (data.success) {
          setSupplier({ id: data.data.id, name: data.data.name, code: data.data.code, status: data.data.status, tags: data.data.tags })
        }
      } catch {}
    }
    const loadRules = async () => {
      try {
        const res = await fetch(`/api/supplier-rulesets?supplier_id=${supplierId}`)
        const data = await res.json()
        setRules(data.data || [])
      } catch {}
    }
  ...
  const saveRule = async () => {
    try {
      ...
      setRules(list.data || [])
      setRuleName(''); setRuleOrder(0); setRuleBlocking(false)
    } catch (e: any) {
          ...
          <TabsContent value="rules" className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <Card className="border-0 shadow-sm lg:col-span-2">
                <CardHeader><CardTitle className="text-sm">Create Rule</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="flex gap-3"><Button variant="outline" onClick={synthesizeRule}><FileCog className="h-4 w-4 mr-2" />Generate with AI</Button><Button onClick={saveRule}><CheckCircle2 className="h-4 w-4 mr-2" />Save Rule</Button></div>
