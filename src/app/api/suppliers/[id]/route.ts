  ...
  try {
    ...
    const [locRes, spRes] = await Promise.all([
      dbQuery<{ count: string }>(
        'SELECT COUNT(*)::int as count FROM core.stock_location WHERE supplier_id = $1',
        [id]
      ),
      dbQuery<{ count: string }>(
        'SELECT COUNT(*)::int as count FROM core.supplier_product WHERE supplier_id = $1',
        [id]
      ),
    ]);
    const hasDependencies =
      parseInt(locRes.rows[0]?.count || '0') > 0 || parseInt(spRes.rows[0]?.count || '0') > 0;
    ...
    if (
      ...
      try {
        ...
        return NextResponse.json({
          success: true,
          message: 'Supplier deactivated (linked records prevented deletion)',
        });
      } catch (e) {
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to delete supplier', message: message || 'Unknown error' },
      { status: 500 }
    );
  }
}
