...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const out = await withTransaction(async client => {
      ...
      const ensureQty = (n: number) => {
        if (!Number.isFinite(n) || n < 0) throw new Error('INVALID_QTY');
      };
      ensureQty(qty);
      ...
      switch (action) {
        case 'allocate_to_supplier':
          ...
          newReserved += qty;
          await client.query(
            `INSERT INTO inventory_allocations (org_id, inventory_item_id, supplier_id, allocation_type, allocated_qty, expires_at, notes)
                              VALUES ($1,$2,$3,'allocation',$4,$5,$6)`,
            [orgId, itemId, supplierId, qty, expiresAt || null, notes || null]
          );
          await client.query(
            `INSERT INTO stock_movements (item_id, type, quantity, reason, reference, timestamp)
                              VALUES ($1,'adjustment',$2,'allocate_to_supplier',$3,NOW())`,
            [itemId, qty, supplierId]
          );
          break;
        case 'deallocate_from_supplier':
          ...
          await client.query(
            `INSERT INTO stock_movements (item_id, type, quantity, reason, reference, timestamp)
                              VALUES ($1,'adjustment',-$2,'deallocate_from_supplier',$3,NOW())`,
            [itemId, qty, supplierId]
          );
          break;
        case 'transfer_allocation':
          ...
          await client.query(
            `INSERT INTO stock_movements (item_id, type, quantity, reason, reference, timestamp)
                              VALUES ($1,'transfer',0,'transfer_allocation',$2,NOW())`,
            [itemId, `${supplierId}->${body?.toSupplierId}`]
          );
          break;
        case 'consignment_in':
          newStock += qty;
          await client.query(
            `INSERT INTO inventory_allocations (org_id, inventory_item_id, supplier_id, allocation_type, allocated_qty, expires_at, notes)
                              VALUES ($1,$2,$3,'consignment',$4,$5,$6)`,
            [orgId, itemId, supplierId, qty, expiresAt || null, notes || null]
          );
          await client.query(
            `INSERT INTO stock_movements (item_id, type, quantity, reason, reference, timestamp)
                              VALUES ($1,'in',$2,'consignment_in',$3,NOW())`,
            [itemId, qty, supplierId]
          );
          break;
        case 'consignment_out':
          ...
          await client.query(
            `INSERT INTO stock_movements (item_id, type, quantity, reason, reference, timestamp)
                              VALUES ($1,'out',$2,'consignment_out',$3,NOW())`,
            [itemId, qty, supplierId]
          );
          break;
      }
      ...
      await client.query(
        `UPDATE inventory_item SET stock_qty=$1, reserved_qty=$2, updated_at=NOW() WHERE id=$3`,
        [newStock, newReserved, itemId]
      );
    ...
    return NextResponse.json(
      { success: false, error: 'ALLOCATION_FAILED', detail: msg },
      { status: 400 }
    );
  }
}
