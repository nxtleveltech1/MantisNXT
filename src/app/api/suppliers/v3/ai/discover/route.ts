...
import type { AISupplierDiscovery, APIResponse } from '@/lib/suppliers/types/SupplierDomain';
...
const EnrichmentRequestSchema = z.object({
  ...
  fields: z
    .array(z.enum(['financial', 'risk', 'sustainability', 'market_position', 'competitors']))
    .optional(),
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newMatches = aiMatches.filter(
      match =>
        !existingSuppliers.suppliers.some(
          existing =>
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = {
      ...
      popularLocations: ['South Africa', 'Germany', 'United States', 'China', 'United Kingdom'],
      searchTips: [
