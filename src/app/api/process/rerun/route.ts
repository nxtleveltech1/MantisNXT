...
export async function POST(request: NextRequest) {
  try {
    ...
    const { position } = await extractionQueue.enqueue(job_id, upload_id, extraction_config, org_id, 0)
    return NextResponse.json({ success: true, job_id, status: 'queued', position }, { status: 202 })
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error?.message || 'Failed to rerun extraction' }, { status: 500 })
  }
}