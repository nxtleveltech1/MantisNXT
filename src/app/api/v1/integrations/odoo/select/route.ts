  ...
  try {
    ...
    if (!entity || !ids.length || !user) {
      return NextResponse.json(
        { success: false, error: 'Invalid selection request' },
        { status: 400 }
      );
    }
    const res = await query(
      `SELECT public.odoo_queue_select($1, $2::text[], $3, $4::uuid) AS updated`,
      [entity, ids, selected, user]
    );
    return NextResponse.json({ success: true, data: { updated: res.rows[0]?.updated || 0 } });
  } catch (error: any) {
    return NextResponse.json(
      { success: false, error: error.message || 'Selection update failed' },
      { status: 500 }
    );
  }
}
