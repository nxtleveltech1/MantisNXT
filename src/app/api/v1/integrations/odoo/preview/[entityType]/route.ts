...
async function handlePreview(
  ...
  try {
    ...
    if (normalizedUrl.includes('www.odoo.sh/project')) {
      console.warn(`[Preview] WARNING: URL format looks like Odoo.sh project URL: ${normalizedUrl}`);
      console.warn(`[Preview] This may not work. For Odoo.sh, use format: https://yourcompany.odoo.sh`);
    ...
    if (normalizedUrl.includes('.odoo.sh')) {
      ...
      if (dbMatch) {
        console.log(`[Preview] Detected Odoo.sh database name with trailing numbers: ${normalizedDatabaseName}`);
    ...
    const modelMap: Record<string, { model: string; fields: string[] }> = {
      products: {
        ...
        fields: ['id', 'name', 'default_code', 'list_price', 'standard_price', 'qty_available', 'type', 'categ_id'],
      },
      orders: {
        model: 'sale.order',
      ...
      customers: {
        model: 'res.partner',
        fields: ['id', 'name', 'email', 'phone', 'is_company', 'customer_rank', 'supplier_rank', 'street', 'city', 'country_id'],
      },
      invoices: {
        model: 'account.move',
        fields: ['id', 'name', 'partner_id', 'invoice_date', 'amount_total', 'state', 'move_type', 'invoice_line_ids'],
      },
    };
    ...
    try {
      ...
      if (previewError.message?.includes('database') && previewError.message?.includes('does not exist')) {
      ...
      if (previewError.message?.includes('Unknown XML-RPC tag') || 
