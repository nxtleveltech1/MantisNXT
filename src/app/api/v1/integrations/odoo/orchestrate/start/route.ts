...
export async function POST(request: NextRequest) {
  try {
    ...
    const entityTypes = Array.isArray(body.entityTypes) ? body.entityTypes : ['customers','products']
    const systems = ['odoo']
    const config = {
      ...
      interBatchDelayMs: Math.min(Math.max(parseInt(String(body.interBatchDelayMs || '2000'), 10), 0), 60000),
    }
    if (!orgId) {
    ...
    return NextResponse.json({ success: true, data: { syncId: res.rows[0]?.sync_id } }, { status: 202 })
  } catch (error: any) {
