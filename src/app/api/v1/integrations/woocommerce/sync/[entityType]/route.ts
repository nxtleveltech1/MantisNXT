  ...
  try {
    ...
    if (lastSyncAt && now - lastSyncAt < 30_000) {
      const res = NextResponse.json(
        { success: false, error: 'Rate limited: please wait before next sync' },
        { status: 429 }
      );
    ...
    if (entityType === 'orders') {
      ...
      result = await sync.syncProductsFromWooCommerce(woo, {
        continueOnError: true,
        dryRun: false,
      });
    } else if (entityType === 'customers') {
      result = await sync.syncCustomersWithWooCommerce(woo, 'inbound', undefined, {
        continueOnError: true,
        dryRun: false,
      });
    } else if (entityType === 'categories') {
      result = await sync.syncCategoriesFromWooCommerce(woo, {
        continueOnError: true,
        dryRun: false,
      });
    }
