...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isUuid(orgId)) return NextResponse.json({ success: false, error: 'orgId required' }, { status: 400 })

    const body = await request.json().catch(() => ({}))
    const entities: string[] = Array.isArray(body?.entities) && body.entities.length ? body.entities : ['customers', 'products', 'orders', 'categories']
    ...
    for (const entity of entities) {
      let data: any[] = []
      if (entity === 'customers') {
        data = await woo.fetchAllPages(p => woo.getCustomers(p), { per_page: 100, order: 'desc', orderby: 'registered_date' })
      } else if (entity === 'products') {
        data = await woo.fetchAllPages(p => woo.getProducts(p), { per_page: 100 })
      } else if (entity === 'orders') {
        data = await woo.fetchAllPages(p => woo.getOrders(p), { per_page: 100, order: 'desc', orderby: 'date' })
      ...
      const typeMap: Record<string, string> = { customers: 'customer', products: 'product', orders: 'order', categories: 'category' }
      ...
      for (const item of data) {
        ...
        const display = (() => {
          ...
          if (entity === 'customers') return { email: item.email, name: `${item.first_name || ''} ${item.last_name || ''}`.trim() }
