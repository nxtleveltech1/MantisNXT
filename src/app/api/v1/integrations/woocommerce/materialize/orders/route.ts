...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!orgIdHeader) return NextResponse.json({ success: false, error: 'orgId required' }, { status: 400 })
    const limiter = getRateLimiter(`materialize:orders:org:${orgIdHeader}`, 10, 0.167)
    if (!limiter.tryConsume(1)) return NextResponse.json({ success: false, error: 'Rate limit exceeded' }, { status: 429 })
    ...
    if (String(row.org_id) !== String(orgIdHeader)) return NextResponse.json({ success: false, error: 'Forbidden' }, { status: 403 })
