...
export async function POST(request: NextRequest) {
  try {
    ...
    const run = await PricingOptimizationService.startOptimization(orgId, validatedData);

    return NextResponse.json(
      { success: true, data: run, message: 'Optimization started successfully' },
      { status: 201 }
    );
  } catch (error) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const runs = await PricingOptimizationService.getAllRuns(orgId, limit);
