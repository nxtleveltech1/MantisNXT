...
export async function GET(request: NextRequest) {
  try {
    ...
    const matches = await service.list(orgId, { status: status ?? undefined, competitorId: competitorId ?? undefined })
    return NextResponse.json({ data: matches, error: null })
  } catch (error) {
    return NextResponse.json(
      { data: null, error: error instanceof Error ? error.message : 'Failed to list product matches' },
      { status: 400 },
    )
  }
}
