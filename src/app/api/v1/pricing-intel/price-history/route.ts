...
export async function GET(request: NextRequest) {
  try {
    ...
    const start = validated.startDate ? new Date(validated.startDate) : new Date(Date.now() - 90 * 24 * 60 * 60 * 1000)
    ...
    const competitorPriceHistoryMap = new Map<string, Map<string, { price: number; date: string }>>()
    ...
    competitorPriceHistoryMap.forEach((competitorMap, date) => {
      competitorMap.forEach((data, competitorName) => {
        const snapshot = competitorSnapshots.rows.find(
          (s) => s.competitor_name === competitorName && s.observed_at.toISOString().split('T')[0] === date
        )
