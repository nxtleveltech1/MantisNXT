...
export async function createService(orgId: string, label: string, key?: string): Promise<AIServiceRow> {
  const resolved = resolveOrgId(orgId);
  const slug = key || label.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'service';
...
export async function updateService(orgId: string, id: string, updates: Partial<{ label: string; key: string; is_enabled: boolean }>): Promise<AIServiceRow | null> {
  ...
  if (updates.is_enabled !== undefined) { sets.push(`is_enabled = $${i++}`); values.push(updates.is_enabled); }
  if (sets.length === 0) {
    const { rows } = await query<AIServiceRow>(`SELECT * FROM ai_service WHERE org_id = $1 AND id = $2`, [resolved, id]);
    if (!rows.length) return null;
    const r = rows[0];
    return { ...r, created_at: iso(r.created_at), updated_at: iso(r.updated_at) };
  }
  const { rows } = await query<AIServiceRow>(`UPDATE ai_service SET ${sets.join(', ')}, updated_at = NOW() WHERE org_id = $1 AND id = $2 RETURNING *`, values);
  if (!rows.length) return null;
  const r = rows[0];
  return { ...r, created_at: iso(r.created_at), updated_at: iso(r.updated_at) };
}
