...
async function generateRecentActivities(limit: number = 20) {
  try {
    // Get recent supplier additions/updates
    ...
    const inventoryActivitiesQuery = `
      SELECT
        ...
        'Inventory updated: ' || i.name || ' (Stock: ' || i.stock_qty || ')' as description
    ...
    const newInventoryQuery = `
      SELECT
        ...
        'New item added: ' || i.name || ' (' || i.sku || ')' as description
      FROM inventory_items i
...
export async function GET(request: NextRequest) {
  try {
    ...
    const activities = await generateRecentActivities(validatedParams.limit + validatedParams.offset)
