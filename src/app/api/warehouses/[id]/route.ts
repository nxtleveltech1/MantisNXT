...
export async function PUT(request: NextRequest, { params }: { params: Promise<Params> }) {
  try {
    ...
    if (validatedData.code) {
      const existingWarehouse = mockWarehouseData.find(
        w => w.code === validatedData.code && w.id !== id
      );
    ...
    const updatedWarehouse = {
      ...
      address: validatedData.address
        ? { ...existingWarehouse.address, ...validatedData.address }
        : existingWarehouse.address,
      capacity: validatedData.capacity
        ? { ...existingWarehouse.capacity, ...validatedData.capacity }
        : existingWarehouse.capacity,
      operatingHours: validatedData.operatingHours
        ? { ...existingWarehouse.operatingHours, ...validatedData.operatingHours }
        : existingWarehouse.operatingHours,
      contactInfo: validatedData.contactInfo
        ? { ...existingWarehouse.contactInfo, ...validatedData.contactInfo }
        : existingWarehouse.contactInfo,
      updatedAt: new Date(),
    };
...
export async function POST(request: NextRequest, { params }: { params: Promise<Params> }) {
  try {
    ...
    const totalZoneCapacity =
      warehouse.zones.reduce((sum, z) => sum + z.capacity.volume, 0) +
      validatedData.capacity.volume;
