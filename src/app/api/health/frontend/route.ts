...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const unhealthyServices = Object.values(
      healthChecks.services.database.status === 'unhealthy' ? [healthChecks.services.database] : []
    ...
    if (slowApis.length > 0) {
      recommendations.push(
        `Slow API responses detected in: ${slowApis.join(', ')} - consider optimizing queries or increasing server resources`
      );
    }

    if (
      healthChecks.system.memoryUsage.heapUsed / healthChecks.system.memoryUsage.heapTotal >
      0.9
    ) {
