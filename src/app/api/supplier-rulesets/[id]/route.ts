...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (validated.supplier_id) {
      ...
      values.push(validated.supplier_id);
    }
    if (validated.rule_name) {
      fields.push(`rule_name = $${idx++}`);
      values.push(validated.rule_name);
    }
    if (validated.rule_type) {
      fields.push(`rule_type = $${idx++}`);
      values.push(validated.rule_type);
    }
    if (validated.trigger_event) {
      fields.push(`trigger_event = $${idx++}`);
      values.push(validated.trigger_event);
    }
    if (validated.execution_order != null) {
      fields.push(`execution_order = $${idx++}`);
      values.push(validated.execution_order);
    }
    if (validated.rule_config) {
      fields.push(`rule_config = $${idx++}::jsonb`);
      values.push(JSON.stringify(validated.rule_config));
    }
    if (validated.is_blocking != null) {
      fields.push(`is_blocking = $${idx++}`);
      values.push(validated.is_blocking);
    }
