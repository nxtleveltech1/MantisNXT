...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!jsonMatch) {
      return NextResponse.json({ success: false, error: 'AI did not return JSON' }, { status: 422 })
    }
    ...
    return NextResponse.json({ success: false, error: error?.message || 'Failed to synthesize rule' }, { status: 400 })
  }
}