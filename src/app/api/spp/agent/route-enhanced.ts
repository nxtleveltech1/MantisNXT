...
async function aiExtractRows(buffer: Buffer, filename: string): Promise<any[]> {
  ...
  try {
    ...
    let prompt =
      'Extract supplier pricelist rows from messy spreadsheet. Provide JSON array of objects with fields: supplier_sku, name, brand, uom, pack_size, price, currency, category_raw, vat_code, barcode.';
    prompt += `\nFilename: ${filename}`;
    if (isExcel) {
    ...
    if (action === 'upload_and_validate') {
      ...
      try {
        if (isAIEnabled()) {
          ...
          let prompt =
            'Analyze supplier pricelist structure and propose robust extraction plan. File: ' +
            file.name;
          const buf = Buffer.from(await file.arrayBuffer());
          if (isExcel) {
        ...
        await auditFinish(reviewFailId, 'failed', {
      ...
      return NextResponse.json({
        success: true,
        data: { upload_id, validation: { status: 'pending' } },
      });
    }
