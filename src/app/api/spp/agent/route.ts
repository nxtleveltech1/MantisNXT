...
async function aiExtractRows(
  ...
  try {
    ...
    let prompt =
      'Extract supplier pricelist rows from this document. Provide JSON array of objects with fields: supplier_sku, name, brand, uom, pack_size, cost_price_ex_vat, price_incl_vat, vat_rate, currency, category_raw, stock_on_hand, barcode.';
    prompt += `\nFilename: ${filename}`;
    ...
    const extractionTimeout =
      timeoutMs ?? (isPDF ? 180000 : parseInt(process.env.AI_EXTRACTION_TIMEOUT_MS || '120000'));
...
async function aiCorrectErrors(
  ...
  try {
    console.log(
      `🤖 Starting AI error correction for ${filename} (${validationErrors.length} errors)`
    );
    const ai = new AIService();

    // Group errors by row number
    const errorsByRow = new Map<number, unknown[]>();
...
async function validateRows(
  ...
  supplierId: string,
  uploadId: string
): Promise<{ valid: unknown[]; errors: unknown[]; warnings: unknown[] }> {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (const rule of validationRules) {
      const config = rule.ruleConfig as unknown;
  ...
  try {
        ...
        for (const execution of rulesResult.executionLog) {
          await logRuleExecution(
            ...
            upload_id,
            {
              ...
              executionOrder: execution.ruleId,
              supplierId: upload.supplier_id,
              triggerEvent: 'pricelist_upload',
              ruleConfig: {},
              isBlocking: execution.blocked,
            },
            execution
          );
        }
    ...
    if (action === 'upload_and_validate') {
      if (!file || !supplier_id) {
        ...
        return NextResponse.json(
          { success: false, error: 'file and supplier_id required' },
          { status: 400 }
        );
      }
      ...
      await auditFinish(sessionId, 'started', {
        filename: file.name,
        size: (file as unknown).size,
        mime: (file as unknown).type,
      });
      ...
      try {
        ...
        if (isPDF && isAIEnabled() && allow_ai_fallback) {
          ...
          if (aiRows && aiRows.length > 0) {
            ...
            const mapped = aiRows.map((row: unknown, idx: number) => {
              ...
              const attrs: unknown = {};
              if (row.cost_excluding !== undefined)
                attrs.cost_excluding = parsePrice(row.cost_excluding);
              if (row.cost_including !== undefined)
                attrs.cost_including = parsePrice(row.cost_including);
        ...
        const getColumn = (row: unknown, names: string[]) => {
        ...
        const mapped = (vatResult2?.rows || []).map((row: unknown, idx: number) => {
          if (usedJoin) {
            const costPrice =
              row['cost_price_ex_vat'] || parsePrice(row['priceExVat'] ?? row['price_ex_vat']) || 0;
            const attrs: unknown = {};

            // Extract cost_excluding, cost_including, and rsp from raw_data if available
            if (row['_cost_excluding'] !== undefined) {
              attrs.cost_excluding =
                typeof row['_cost_excluding'] === 'number'
                  ? row['_cost_excluding']
                  : parsePrice(row['_cost_excluding']);
            }
            if (row['_cost_including'] !== undefined) {
              attrs.cost_including =
                typeof row['_cost_including'] === 'number'
                  ? row['_cost_including']
                  : parsePrice(row['_cost_including']);
            }
            ...
            const priceValue = getColumn(row, [
              'COST  EX VAT',
              'COST EX VAT',
              'Cost Ex VAT',
              'Cost',
            ...
            if (getColumn(row, ['QTY ON ORDER']))
              attrs.qty_on_order = parseInt(String(getColumn(row, ['QTY ON ORDER']))) || 0;
            if (getColumn(row, ['NEXT SHIPMENT']))
              attrs.next_shipment = getColumn(row, ['NEXT SHIPMENT']);
            if (getColumn(row, ['Tags'])) attrs.tags = getColumn(row, ['Tags']);
            if (getColumn(row, ['Links'])) attrs.links = getColumn(row, ['Links']);
            if (getColumn(row, ['Brand Sub Tag']))
              attrs.brand_sub_tag = getColumn(row, ['Brand Sub Tag']);
            ...
            const costExcludingValue = getColumn(row, [
              'Cost Excluding',
              'COST EXCLUDING',
              'Cost Ex VAT',
              'COST EX VAT',
            ]);
            const costIncludingValue = getColumn(row, [
              'Cost Including',
              'COST INCLUDING',
              'Cost Inc VAT',
              'COST INC VAT',
            ]);
            const rspValue = getColumn(row, [
            ...
            const costPrice =
              row['cost_price_ex_vat'] || attrs.cost_excluding || parsePrice(priceValue) || 0;
            return {
              upload_id,
              row_num: idx + 1,
        ...
        if (validation.status !== 'valid' && validation.status !== 'warning' && isAIEnabled()) {
          ...
          if (errorCount > 0) {
            ...
            const correctionAuditId = await auditStart(
              supplier_id,
              upload_id,
              'ai_error_correction'
            );
            ...
            try {
              ...
              if (correctedRows.length > 0) {
                ...
                const vatAuditId3 = await auditStart(
                  supplier_id,
                  upload_id,
                  'vat_normalization_corrected'
                );
                ...
                const errorRowNums = [
                  ...new Set((validation.errors || []).map((e: unknown) => e.row_num)),
                ];
                ...
                const mappedCorrected = (vatResult3?.rows || []).map(
                ...
                const correctedInserted = await pricelistService.insertRows(
                  upload_id!,
                  mappedCorrected
                );
                ...
                const revalidationId = await auditStart(
                  supplier_id,
                  upload_id,
                  'revalidation_after_correction'
                );
          ...
          console.warn(
            `⚠️ Validation failed with ${validation.errors?.length || 0} errors, but AI is not enabled`
          );
        }
