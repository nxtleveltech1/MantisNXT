...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!orgIdValue || !uuidRegex.test(orgIdValue.toString())) {
      return NextResponse.json(
        { success: false, error: 'org_id must be a valid UUID (supply via header x-org-id or form field org_id).' },
        { status: 400 },
      );
    }
    ...
    const serviceIdValue =
      req.headers.get('x-ai-service-id') || formData.get('service_id');
    const serviceId =
      serviceIdValue && uuidRegex.test(serviceIdValue.toString())
        ? serviceIdValue.toString()
        : undefined;
    const serviceName =
      (formData.get('service_name') || req.headers.get('x-ai-service-name') || process.env.AI_PRICE_EXTRACTION_SERVICE_NAME)?.toString() ||
