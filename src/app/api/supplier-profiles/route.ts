...
export async function GET(request: NextRequest) {
  try {
    ...
    if (profileName) { sql += ` AND profile_name = $${idx++}`; params.push(profileName) }
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: error?.message || 'Failed to upsert profile' }, { status: 400 })
  }
}