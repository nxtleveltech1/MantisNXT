...
export async function GET() {
  try {
    ...
    const coreList = coreRes.rows.map(r => ({
      id: r.category_id,
      category_id: r.category_id,
      name: r.name,
      source: 'core' as const,
    }));
    const rawList = rawRes.rows.map(r => ({
      id: r.id,
      category_id: null as unknown,
      name: r.name,
      source: 'raw' as const,
    }));
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      {
        success: false,
        error: 'Invalid request. Provide action="seed_av_categories" or categories array',
      },
      { status: 400 }
    );
  } catch (error) {
