...
export const GET = ApiMiddleware.withAuth(
  async (request: NextRequest, context: RequestContext) => {
    try {
      ...
      if (!id) {
        return ApiMiddleware.createErrorResponse('Supplier ID is required', 400);
      }

      const supplier = mockSupplierData.find(supplier => supplier.id === id);
      if (!supplier) {
        return ApiMiddleware.createErrorResponse('Supplier not found', 404, { requestedId: id });
      }
  ...
  async (request: NextRequest, context: RequestContext) => {
    try {
      ...
      if (!validation.success) {
        return ApiMiddleware.createErrorResponse('Validation error', 400, {
      ...
      if (validatedData.code) {
        const existingSupplier = mockSupplierData.find(
          supplier => supplier.code === validatedData.code && supplier.id !== id
        );
        if (existingSupplier) {
          return ApiMiddleware.createErrorResponse('Supplier code already exists', 409, {
      ...
      if (validatedData.email) {
        const existingEmail = mockSupplierData.find(
          supplier => supplier.email === validatedData.email && supplier.id !== id
        );
        if (existingEmail) {
          return ApiMiddleware.createErrorResponse('Supplier email already exists', 409, {
      ...
      if (validatedData.contacts) {
        ...
        if (primaryContacts.length > 1) {
          return ApiMiddleware.createErrorResponse('Only one primary contact is allowed', 400);
        }
      ...
      const updatedSupplier: InternalSupplier = {
        ...
        paymentTerms: validatedData.paymentTerms
          ? { ...existingSupplier.paymentTerms, ...validatedData.paymentTerms }
          : existingSupplier.paymentTerms,
        performance: validatedData.performance
          ? { ...existingSupplier.performance, ...validatedData.performance }
        ...
        customFields: validatedData.customFields
          ? { ...existingSupplier.customFields, ...validatedData.customFields }
      ...
      const auditEntry = {
        ...
        previousValues: Object.fromEntries(
          changedFields.map(field => [
            field,
            existingSupplier[field as keyof typeof existingSupplier],
          ])
        ),
  ...
  async (request: NextRequest, context: RequestContext) => {
    try {
      ...
      if (blockers.length > 0 && !force && !soft) {
        return ApiMiddleware.createErrorResponse('Cannot delete supplier', 400, {
          blockers,
          suggestion:
            'Use ?force=true to force deletion, ?soft=true for soft delete, or resolve the blocking conditions first',
        });
      }
