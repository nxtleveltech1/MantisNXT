...
const AnalysisTypeSchema = z.enum(['business', 'inventory', 'financial', 'operational', 'custom']);
...
const postHandler = ApiMiddleware.withValidation(
  ...
  try {
    ...
    const datasetSummary = buildDatasetSummary(
      payload.dataSources,
      payload.includeComparisons ? payload.comparativePeriod : undefined
    );
    ...
    const report =
      payload.output?.format === 'report'
        ? buildStructuredReport(result.data?.text ?? '', visualizationSuggestions, payload)
        : undefined;
    ...
    const record = storeAnalysisHistory({
      ...
      recommendations,
      visualizations:
        payload.output?.includeVisualizations === false ? [] : visualizationSuggestions,
      schedule,
      alerts: payload.alerts ?? null,
      report,
    });
...
function buildAnalysisPrompt(
  ...
  const lines: string[] = [];
  lines.push(
    `You are an enterprise intelligence analyst generating a ${payload.analysisType} review.`
  );
  ...
  if (payload.includeComparisons && payload.comparativePeriod) {
    lines.push(
      'Include comparative analysis between the primary period and comparative period provided.'
    );
  }
  ...
  if (payload.output?.format === 'report') {
    lines.push(
      'Produce a report with the following sections: Executive Summary, KPI Analysis, Risks & Opportunities, Recommendations, Data Appendix.'
    );
  } else if (payload.output?.format === 'json') {
    lines.push(
      'Return a JSON object with keys: summary, kpis, opportunities, risks, recommendations, alerts.'
    );
  ...
  lines.push(
    'Ensure the analysis is data-driven, references specific metrics, and provides clear next actions.'
  );
  return lines.join('\n');
}
...
async function generateRecommendations(analysisText: string, payload: AnalysisRequest) {
  ...
  const messages: AIChatMessage[] = [
    {
      role: 'system',
      content:
        'You are an operations strategist. Provide concise, actionable recommendations based on the analysis.',
    },
    {
      role: 'user',
  ...
  const response = await chatService.chat(messages, {
    metadata: { analysisType: payload.analysisType },
  });
  ...
  streamingResult.summary
    .then(async summary => {
      ...
      storeAnalysisHistory({
        ...
        recommendations,
        visualizations:
          context.payload.output?.includeVisualizations === false
            ? []
...
function toAnalysisEventStream(
  ...
  info: {
    promptHash: string;
    analysisType: AnalysisType;
    dataset: ReturnType<typeof buildDatasetSummary>['preview'];
  }
