...
export default function SupplierProfilesPage() {
  ...
  const crumbs = pathname.startsWith('/nxt-spp')
    ...
    : [
        { label: 'Administration', href: '/admin/settings/general' },
  ...
  const guidelines = useMemo(() => {
    try {
      return JSON.parse(guidelinesText);
    } catch {
      return null;
    }
  }, [guidelinesText]);
  ...
  const onSave = async () => {
    ...
    if (!guidelines || !processing || !quality || !compliance) {
    ...
    const list = await fetch(`/api/supplier-profiles?supplier_id=${supplierId}`).then(r =>
      r.json()
    );
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-muted-foreground text-sm">
                  Guidelines, processing config, quality standards and compliance
                </div>
              </div>
            </div>
            <div className="flex items-center gap-3">
                ...
                <div className="divide-y">
                  {profiles.map(p => (
                    <button
                      key={p.id}
                      onClick={() => onSelect(p)}
                      className="hover:bg-muted/50 w-full p-4 text-left"
                    >
                  ...
                  {profiles.length === 0 && (
                    <div className="text-muted-foreground p-6 text-sm">
                      No profiles found for supplier
                    </div>
                  )}
                </div>
              </ScrollArea>
          ...
          <Card className="border-0 shadow-sm lg:col-span-2">
            <CardHeader>
              <CardTitle className="text-sm">
                {selected ? 'Edit Profile' : 'Create/Update Profile'}
              </CardTitle>
            </CardHeader>
