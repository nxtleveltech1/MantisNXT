...
export function AIPriceExtractionPanel() {
  ...
  const [serviceName, setServiceName] = useState<string>(
    process.env.NEXT_PUBLIC_AI_SUPPLIER_EXTRACTION_SERVICE_NAME ||
      'Supplier Pricelist Data Extraction'
  );
    ...
    <div className="space-y-6">
      <Card className="border-primary/10 shadow-md">
        <CardHeader className="flex flex-col gap-2 border-b bg-gradient-to-r from-slate-50 via-white to-indigo-50">
          ...
          <CardDescription>
            Upload messy supplier spreadsheets; AI normalizes SKUs, currency, pricing, UOM, pack
            sizes, and lead times into production-ready rows.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 pt-6">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-3">
              ...
              >
                <label className="text-muted-foreground flex w-full cursor-pointer flex-col items-center justify-center gap-2 p-6 text-center text-sm">
                  <UploadCloud className="text-primary h-6 w-6" />
                  <span className="text-foreground font-medium">Drop Excel/CSV here or browse</span>
                  <span className="text-muted-foreground text-xs">
                    We parse in-memory; nothing is stored until you decide.
                  </span>
                  <Input
                    id="file"
                    type="file"
      ...
      {hasResult && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Review before merging into Supplier Inventory Portfolio.
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <thead className="bg-muted/60 text-left">
                  <tr>
                    {resultHeaders.map(header => (
                      <th
                        key={header}
                        className="text-muted-foreground px-3 py-2 font-semibold capitalize"
                      >
                        {header.replace(/_/g, ' ')}
                      </th>
                    ))}
