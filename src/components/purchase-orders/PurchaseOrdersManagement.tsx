...
const PurchaseOrdersManagement: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    if (databaseOrders && databaseOrders.length > 0) {
      ...
      const inProgress = databaseOrders.filter(po =>
        ['sent', 'acknowledged', 'in_progress', 'shipped'].includes(po.status)
      ).length;
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => setIsBulkOperationsOpen(true)}>
                    ...
                    <TableRow key={order.id}>
                      ...
                      <TableCell>
                        <div className="flex flex-col gap-1">
                          <Progress
                            value={getOrderProgressFromStatus(order.status)}
                            className="w-20"
                          />
      ...
      {/* Modals and Dialogs */}
      <POCreationWizard open={isWizardOpen} onClose={() => setIsWizardOpen(false)} />
      ...
      <POTemplates open={isTemplatesOpen} onClose={() => setIsTemplatesOpen(false)} />
