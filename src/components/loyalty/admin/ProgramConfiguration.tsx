...
export default function ProgramConfiguration({
  ...
  const getStepFields = (step: number): string[] => {
    switch (step) {
      case 1:
        return ['name', 'description', 'earn_rate', 'is_active', 'is_default'];
      case 2:
        ...
        {STEPS.map((step, idx) => {
          ...
          return (
            <React.Fragment key={step.id}>
              ...
              {idx < STEPS.length - 1 && (
                <div className={`mx-4 h-0.5 flex-1 ${isCompleted ? 'bg-primary' : 'bg-muted'}`} />
              )}
            </React.Fragment>
          );
        })}
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              {currentStep === 2 && (
                <div className="space-y-4">
                  {(['bronze', 'silver', 'gold', 'platinum', 'diamond'] as LoyaltyTier[]).map(
                    tier => (
                      <FormField
                        key={tier}
                        control={form.control}
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="flex items-center gap-2">
                              <div className={`h-4 w-4 rounded-full ${TIER_COLORS[tier]}`} />
              ...
              {currentStep === 5 && (
                <div className="space-y-6">
                  <Alert>
                    ...
                    <AlertDescription>
                      Review your program configuration before saving. You can edit it later if
                      needed.
                    </AlertDescription>
                  </Alert>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <h3 className="mb-2 font-semibold">Tier Configuration</h3>
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex gap-2 text-xs">
                                  ...
                                  {benefits.discount && (
                                    <Badge variant="outline">{benefits.discount}% off</Badge>
                                  )}
                                  {benefits.free_shipping && (
                                    <Badge variant="outline">Free Ship</Badge>
                                  )}
                                </div>
                              </div>
                            );
