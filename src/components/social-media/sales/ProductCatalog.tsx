...
export default function ProductCatalog({ conversationId, phoneNumber, channelId }: ProductCatalogProps) {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch("/api/social-media/sales/products")
      if (response.ok) {
  ...
  const fetchCart = async () => {
    ...
    try {
      const response = await fetch(`/api/social-media/sales/cart?conversationId=${conversationId}`)
      if (response.ok) {
        const data = await response.json()
  ...
  const handleAddToCart = async (product: Product) => {
    ...
    try {
      const response = await fetch("/api/social-media/sales/cart", {
        method: "POST",
  ...
  const handleSendOrder = async (orderPhoneNumber?: string) => {
    ...
    if (!finalPhoneNumber && !conversationId) {
      alert("Please provide a phone number to send the order.")
      return
    }

    try {
      const response = await fetch("/api/social-media/sales/orders", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          conversationId,
      ...
      if (response.ok) {
        ...
        alert(error.error || "Error sending order. Please try again.")
      }
    } catch (error) {
