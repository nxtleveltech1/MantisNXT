...
export default function UnifiedInbox() {
  ...
  const fetchConversations = async () => {
    try {
      setIsLoading(true);
      const response = await fetch("/api/social-media/messages");
  ...
  const fetchMessages = async (conversationId: number) => {
    try {
      const response = await fetch(`/api/social-media/messages?conversationId=${conversationId}`);
      const data = await response.json();
  ...
  const markAsRead = async (conversationId: number) => {
    try {
      await fetch(`/api/social-media/messages/${conversationId}/read`, { method: "POST" });
      // Update local state
