...
const STEPS = [
  ...
  { id: 'locations', title: 'Location Assignment', description: 'Assign warehouse locations' },
  {
    ...
    description: 'Configure supplier-specific settings',
  },
...
export default function ProductToInventoryWizard({
  ...
  const [inventoryConfigs, setInventoryConfigs] = useState<Map<string, InventoryConfiguration>>(
    new Map()
  );
  ...
  const loadInitialData = useCallback(async () => {
    try {
      ...
      const warehousesData = warehousesResponse.ok
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 2: // Location assignment
        return (
          selectedWarehouse !== '' &&
          Array.from(selectedProducts).every(productId => {
            const config = inventoryConfigs.get(productId);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure inventory parameters and promote selected products to your main inventory
            system.
          </DialogDescription>
        </DialogHeader>
                    ...
                    <CardContent>
                      <div className="space-y-6">
                        {getSelectedProducts().map(product => {
                          const config =
                            inventoryConfigs.get(product.id) || ({} as InventoryConfiguration);
                          return (
                            <Card key={product.id} className="border-muted">
                              ...
                              <CardContent>
                                <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                                  <div>
                                    <Label htmlFor={`initialStock-${product.id}`}>
                                      Initial Stock
                                    </Label>
                                    <Input
                                      ...
                                      onChange={e =>
                                        updateInventoryConfig(product.id, {
                                  ...
                                  <div>
                                    <Label htmlFor={`reorderPoint-${product.id}`}>
                                      Reorder Point *
                                    </Label>
                                    <Input
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <div>
                          <p className="text-muted-foreground text-sm">Total Initial Stock Value</p>
                          <p className="text-lg font-semibold">{formatCurrency(getTotalValue())}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground text-sm">Average Unit Cost</p>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Choose the warehouse for these products</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                        <div>
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Assign specific locations for each product</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                        {getSelectedProducts().map(product => {
                      ...
                      <div className="bg-muted/50 mt-4 rounded-lg p-4">
                        <h4 className="mb-2 text-sm font-medium">Location Format</h4>
                        <p className="text-muted-foreground text-sm">
                          Use format: Zone-Aisle-Shelf-Bin (e.g., A-1-2-B for Zone A, Aisle 1, Shelf
                          2, Bin B)
                        </p>
                      </div>
                    </CardContent>
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                      <div className="space-y-4">
                        <div>
                          ...
                          <div className="relative">
                            <Clock className="text-muted-foreground absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 transform" />
                            <Input
                              id="leadTimeDays"
                              type="number"
                              min="1"
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="rounded-lg bg-blue-50 p-4">
                          ...
                          <ul className="space-y-1 text-sm text-blue-800">
                            <li>
                              • Orders will be placed {supplierConfig.leadTimeDays} days before
                              stock runs out
                            </li>
                            <li>• Minimum order: {supplierConfig.minimumOrderQuantity} units</li>
                            {supplierConfig.preferredOrderMultiple > 1 && (
                              <li>
                                • Orders rounded to multiples of{' '}
                                {supplierConfig.preferredOrderMultiple}
                              </li>
                            )}
                            <li>
                              • Auto-reorder:{' '}
                              {supplierConfig.autoReorderEnabled ? 'Enabled' : 'Disabled'}
                            </li>
                          </ul>
                        </div>
                      </div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                          ...
                          <div className="bg-muted/50 rounded-lg p-4 text-center">
                            ...
                            <p className="text-muted-foreground text-sm">Initial Value</p>
                          </div>
                          <div className="bg-muted/50 rounded-lg p-4 text-center">
                            <p className="text-2xl font-bold text-orange-600">
                              {supplierConfig.leadTimeDays}
                            </p>
                            <p className="text-muted-foreground text-sm">Lead Time (Days)</p>
                          </div>
                          <div className="bg-muted/50 rounded-lg p-4 text-center">
                        ...
                        {/* Products to Promote */}
                        <div>
                          <h4 className="mb-3 font-medium">Products to Promote</h4>
                                ...
                                >
                                  ...
                                  <div className="text-right">
                                    <p className="font-medium">
                                      {config.initialStock} units @{' '}
                                      {formatCurrency(config.unitCost)}
                                    </p>
                                    <p className="text-muted-foreground text-sm">
                        ...
                        {promotionResults.length > 0 && (
                          <div>
                            <h4 className="mb-3 font-medium">Promotion Results</h4>
                            <div className="space-y-2">
            ...
            <div className="flex items-center justify-between border-t pt-6">
              ...
              <div className="flex items-center gap-2">
                {currentStep < STEPS.length - 1 ? (
                  ...
                  <Button onClick={executePromotion} disabled={!canProceed() || processing}>
                    {processing ? (
