...
export function BatchCategorizationProgress({
  progress,
  className,
}: BatchCategorizationProgressProps) {
  if (!progress) {
    return null;
  }

  const batchProgress =
    progress.totalBatches > 0 ? (progress.currentBatch / progress.totalBatches) * 100 : 0;

  const productProgress =
    progress.totalProducts > 0 ? (progress.productsProcessed / progress.totalProducts) * 100 : 0;
        ...
        <div className="grid grid-cols-2 gap-4">
          {progress.tokensUsed && (
            <div className="space-y-1">
              ...
              <div className="text-lg font-semibold">{formatTokens(progress.tokensUsed.total)}</div>
              <div className="text-muted-foreground text-xs">
                {formatTokens(progress.tokensUsed.input)} input /{' '}
                {formatTokens(progress.tokensUsed.output)} output
              </div>
            </div>
          )}
