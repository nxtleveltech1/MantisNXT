...
export const TagStatisticsPanel = memo(function TagStatisticsPanel({
  refreshTrigger,
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        alert(
          `Database schema incomplete. Missing columns: ${data.missing_columns?.join(', ')}. Please run migration 0035_ai_tagging_tracking.sql`
        );
      }
      setLoading(false);
    } catch (error) {
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground text-xs">Being monitored</p>
          </CardContent>
        </Card>
