...
export default function ChatAssistant() {
  ...
  async function send() {
    ...
    try {
      ...
      if (assistantText) {
        ...
        setMessages(prev => [
          ...prev,
          { role: 'assistant', content: 'No content returned from provider.' },
        ]);
      }
  ...
  return (
    <Card className="flex h-[calc(100vh-12rem)] max-h-[900px] w-full flex-col">
      ...
      <CardContent className="flex h-full flex-col">
        <div
          ref={scrollRef}
          className="bg-background flex-1 space-y-4 overflow-auto rounded border p-4"
        >
          {messages
            ...
            .map((m, idx) => (
              ...
              >
                {m.role === 'assistant' && <Bot className="mt-1 h-4 w-4 text-blue-600" />}
                <div
                  className={`max-w-[80%] rounded-md px-3 py-2 text-sm whitespace-pre-wrap ${m.role === 'assistant' ? 'bg-blue-50 text-blue-900' : 'bg-gray-100 text-gray-900'}`}
                >
                  {m.content}
                </div>
                {m.role === 'user' && <User className="mt-1 h-4 w-4 text-gray-600" />}
              </div>
            ))}
        </div>
