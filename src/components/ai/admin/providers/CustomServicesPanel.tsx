...
export default function CustomServicesPanel() {
  ...
  const {
    ...
    refetch: refetchCLIAvailability,
    isFetching: isFetchingCLI,
  } = useQuery<Record<string, { available: boolean; version?: string; installationHint?: string }>>(
    {
      ...
      queryFn: async () => {
        try {
          ...
          if (!res.ok) {
            ...
            console.warn(
              '[CustomServicesPanel] CLI availability check failed:',
              res.status,
              errorText
            );
    ...
    (async () => {
      for (const s of services) {
        ...
        try {
          ...
          setConfigs(prev => ({
            ...prev,
            [s.id]: data?.data || { id: '', enabled: s.is_enabled, config: {} },
          }));
        } catch {
  ...
  const updateConfig = useMutation({
    ...
    onSuccess: (_, vars) => {
      ...
      setTimeout(() => {
        void fetch(`/api/v1/ai/services/${vars.serviceId}/config`)
          .then(r => r.json())
          .then(d => {
  ...
  const addProviderInstance = (serviceId: string) => {
    ...
    const effectiveUseOAuth =
      form.provider === 'google' && form.useCLI && form.useOAuth === undefined
        ? true
        : form.useOAuth === true;
    ...
    const newInstance: ProviderInstance = {
      ...
      ...(form.useCLI && (form.provider === 'google' || form.provider === 'openai')
        ? {
            useCLI: true,
            cliCommand:
              form.cliCommand?.trim() || (form.provider === 'google' ? 'gemini' : 'codex'),
            cliArgs: form.cliArgs?.trim()
              ? form.cliArgs
                  .split(',')
                  .map(s => s.trim())
                  .filter(Boolean)
  ...
  return (
    <div className="space-y-4">
      {services.length === 0 ? null : (
        <div className="text-muted-foreground text-sm">Custom AI Services</div>
      )}
      {services.map(s => {
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Label htmlFor={`${s.id}-enabled`} className="text-sm">
                  ...
                  <Switch
                    ...
                    onCheckedChange={checked =>
                      updateConfig.mutate({ serviceId: s.id, payload: { enabled: checked } })
                    }
                  />
                </div>
              </div>
                      ...
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div className="space-y-2">
                          ...
                          <Select
                            ...
                            onValueChange={v =>
                              setForm(s.id, { googleApiVersion: v as 'v1' | 'v1alpha' })
                            }
                          >
                            <SelectTrigger>
                              <SelectValue />
                          ...
                          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div className="space-y-2">
                              ...
                              <Input
                                ...
                                onChange={e =>
                                  setForm(s.id, { cliWorkingDirectory: e.target.value })
                                }
                              />
                            </div>
                          </div>
                            ...
                            <div className="space-y-2">
                              <div className="flex items-center space-x-2">
                                <input
                                  ...
                                  onChange={() =>
                                    setForm(s.id, { useOAuth: true, useGCloudADC: false })
                                  }
                                  className="h-4 w-4"
                                />
                                <Label
                                  htmlFor={`${s.id}-cli-oauth`}
                                  className="cursor-pointer text-sm font-normal"
                                >
                              ...
                              <div className="flex items-center space-x-2">
                                <input
                                  ...
                                  onChange={() =>
                                    setForm(s.id, { useGCloudADC: true, useOAuth: false })
                                  }
                                  className="h-4 w-4"
                                />
                                <Label
                                  htmlFor={`${s.id}-cli-gcloud`}
                                  className="cursor-pointer text-sm font-normal"
                                >
                              ...
                              <div className="flex items-center space-x-2">
                                <input
                                  ...
                                  onChange={() =>
                                    setForm(s.id, { useOAuth: false, useGCloudADC: false })
                                  }
                                  className="h-4 w-4"
                                />
                                <Label
                                  htmlFor={`${s.id}-cli-apikey`}
                                  className="cursor-pointer text-sm font-normal"
                                >
                                  API Key
                                </Label>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              />
                              <Label
                                htmlFor={`${s.id}-openai-cli-apikey`}
                                className="cursor-pointer text-sm font-normal"
                              >
                                API Key
                              </Label>
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={async e => {
                      ...
                      e.stopPropagation();
                      console.log(
                        '[CustomServicesPanel] Test Connection clicked for service:',
                        s.id
                      );
                      const form = getForm(s.id);
                      ...
                      const isWebSearch = isWebSearchProvider(form.provider);
                      const isCLIMode =
                        (form.provider === 'google' || form.provider === 'openai') && form.useCLI;
                      ...
                      console.log(
                        '[CustomServicesPanel] Test connection validation passed, proceeding...'
                      );
                      ...
                      if (form.provider === 'google') {
                        if (form.useVertexAI) {
                          ...
                          if (!form.useOAuth && !form.useGCloudADC && !form.apiKey) {
                            toast.error(
                              'Select OAuth/gcloud ADC or provide an API key for CLI mode'
                            );
                            return;
                          }
                      ...
                      try {
                        ...
                        if (isCLIMode) {
                          ...
                          testConfig.cliArgs = form.cliArgs
                            ? form.cliArgs
                                .split(',')
                                .map((s: string) => s.trim())
                                .filter(Boolean)
                        ...
                        {isEditing ? (
                          ...
                          <div className="space-y-4">
                            ...
                            >
                              ...
                              {!isWebSearchProvider(editForm?.provider || 'google') &&
                                editForm?.provider !== 'google' && (
                                  <div className="space-y-2">
                                    <Label>Model *</Label>
                                    <Input
                              ...
                              {editForm?.provider === 'google' && (
                                <div className="space-y-2">
                                  ...
                                  <Select
                                    ...
                                    onValueChange={v =>
                                      setEditForm(prev => (prev ? { ...prev, model: v } : null))
                                    }
                                  >
                                    <SelectTrigger>
                                      <SelectValue />
                              ...
                              <div className="space-y-4 border-t pt-4">
                                <div className="flex items-center space-x-2">
                                  <input
                                    ...
                                    onChange={e =>
                                      setEditForm(prev =>
                                        prev ? { ...prev, useVertexAI: e.target.checked } : null
                                      )
                                    }
                                    className="rounded"
                                  />
                                        ...
                                        {editForm.useOAuth && (
                                          <div className="mt-2 space-y-2">
                                            <p className="text-muted-foreground text-xs">
                                              ...
                                              <code className="bg-muted rounded px-1">gemini</code>{' '}
                                              in terminal to log in with your Google account (free
                                              or paid).
                                            </p>
                                          </div>
                                        )}
                            ...
                            <div className="flex justify-end gap-2">
                              ...
                              <Button size="sm" onClick={() => saveEditedInstance(s.id, inst.id)}>
                                Save Changes
                              </Button>
                            </div>
                          </div>
                            ...
                            <div className="flex flex-1 items-center gap-4">
                              ...
                              {inst.useCLI && (
                                <div className="text-muted-foreground mt-1 text-xs">
                                  CLI Mode • Command:{' '}
                                  {inst.cliCommand ||
                                    (inst.provider === 'google' ? 'gemini' : 'codex')}
                                  {inst.useOAuth && ' • OAuth'}
                                  {inst.useGCloudADC && ' • gcloud ADC'}
                                </div>
                              )}
                              ...
                              <div className="flex items-center gap-2">
                                <Switch
                                  ...
                                  onCheckedChange={checked =>
                                    toggleProviderInstance(s.id, inst.id, checked)
                                  }
                                />
