...
const getOpenRouterFilterKey = (serviceType: string, providerId: string) =>
  `${serviceType}:${providerId}:openrouterFreeOnly`;

const isOpenRouterProvider = (
  providerId: string,
  section: unknown,
  rootConfig?: AIServiceConfig
): boolean => {
...
export default function AIServiceConfiguration() {
  ...
  const { data: providerPresets = [] } = useQuery<
    { id: string; name: string; provider_type: string; base_url?: string; default_model?: string }[]
  >({
  ...
  const createConfigMutation = useMutation({
    mutationFn: async () => {
      ...
      console.log('[Create Service] Starting with:', {
        label: createForm.displayName,
  ...
  const [customProvidersLocal, setCustomProvidersLocal] = useState<
    Record<string, Record<string, unknown>>
  >({});
  // Local state for web search API keys to enable immediate UI updates
  const [webSearchKeysLocal, setWebSearchKeysLocal] = useState<
    Record<
      string,
      {
        serperApiKey?: string;
  ...
  return (
    <div className="space-y-6">
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Create and configure a new AI service with provider, endpoint, API key, and model.
              </DialogDescription>
            </DialogHeader>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2 md:col-span-2">
                <Label>
                  Service Label <span className="text-red-500">*</span>
                </Label>
                <Input
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={v => setCreateForm(p => ({ ...p, provider: v as unknown }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <Select
                  onValueChange={presetId => {
                    ...
                    if (!preset) return;
                    const prov =
                      preset.provider_type === 'openai' || preset.provider_type === 'anthropic'
                ...
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={
                        providerPresets.length ? 'Choose preset…' : 'No presets available'
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <div className="flex items-center gap-2">
                  <Select
                    ...
                    onValueChange={v => setCreateForm(p => ({ ...p, model: v }))}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent className="max-h-64 overflow-auto">
                      {(
                        dynamicModels[`assistant:${createForm.provider}`] ||
                        MODEL_OPTIONS[createForm.provider] ||
                        []
                      ).map((m: unknown) => (
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      const cfg = {
                        provider: createForm.provider,
                        activeProvider: createForm.provider,
      ...
      <div>
        <h2 className="mb-4 text-xl font-semibold">Built-in AI Services</h2>
        <UnifiedServicePanel />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6" style={{ display: 'none' }}>
        {Object.entries(SERVICE_INFO).map(([serviceType, info]) => {
          ...
          const showOpenRouterFreeOnly = isActiveOpenRouter
            ? !!openRouterFilters[openRouterFilterKey]
            : false;
                    ...
                    <Switch
                      ...
                      onCheckedChange={checked =>
                        updateConfigMutation.mutate({
                          service: serviceType,
                          updates: {
                            config: { ...config?.config, enableMultipleProviders: checked },
                          },
                        })
                      }
                    />
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={async e => {
                                ...
                                if (baseUrl && key) {
                                  await fetchModelsFor(serviceType, {
                                    ...nextCfg,
                                    activeProvider: p,
                                    provider: p,
                                  });
                            ...
                            <div className="flex items-center justify-between gap-2">
                              ...
                              {isSectionOpenRouter && (
                                <div className="flex items-center gap-2">
                                  ...
                                  />
                                  <Label
                                    htmlFor={`${serviceType}-${p}-free-only`}
                            ...
                            >
                              ...
                              <SelectContent className="max-h-64 overflow-auto">
                                {models.length > 0 ? (
                                  ...
                                  <div className="text-muted-foreground px-3 py-2 text-sm">
                                    No models match the current filters. Try disabling the
                                    free-model filter or load models again.
                                  </div>
                                )}
                              </SelectContent>
                            </Select>
                  ...
                  <>
                    ...
                    <Card className="border-2 border-purple-200 bg-purple-50/50">
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                          ...
                          <div className="space-y-2">
                            <Label htmlFor={`${serviceType}-ai-model-manual`}>
                              Or Enter Model Name
                            </Label>
                            <Input
                        ...
                        <p className="text-muted-foreground text-xs">
                          Select a model from the dropdown or manually enter a model name. The model
                          will be used for extracting supplier information from web content.
                        </p>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="border-2 border-blue-200 bg-blue-50/50">
                      <CardHeader>
                        ...
                        <CardDescription>
                          Configure web search providers for supplier discovery. At least one API
                          key is recommended.
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              value={
                                webSearchKeysLocal[serviceType]?.serperApiKey ??
                                config?.config?.webSearch?.serperApiKey ??
                                ''
                              }
                              onChange={e => {
                                const newValue = e.target.value;
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              value={
                                webSearchKeysLocal[serviceType]?.tavilyApiKey ??
                                config?.config?.webSearch?.tavilyApiKey ??
                                ''
                              }
                              onChange={e => {
                                const newValue = e.target.value;
                          ...
                          <div className="space-y-2">
                            <Label htmlFor={`${serviceType}-google-key`}>
                              Google Search API Key
                            </Label>
                            <Input
                              ...
                              value={
                                webSearchKeysLocal[serviceType]?.googleSearchApiKey ??
                                config?.config?.webSearch?.googleSearchApiKey ??
                                ''
                              }
                              onChange={e => {
                                const newValue = e.target.value;
                          ...
                          <div className="space-y-2">
                            <Label htmlFor={`${serviceType}-google-cx`}>
                              Google Search Engine ID
                            </Label>
                            <Input
                              ...
                              value={
                                webSearchKeysLocal[serviceType]?.googleSearchEngineId ??
                                config?.config?.webSearch?.googleSearchEngineId ??
                                ''
                              }
                              onChange={e => {
                                const newValue = e.target.value;
                        ...
                        <div className="rounded-md bg-blue-100 p-3 text-sm text-blue-800">
                          <strong>Note:</strong> If no API keys are configured, the system will use
                          DuckDuckGo (free, no API key required) as a fallback.
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                const currentProviders =
                                  customProvidersLocal[serviceType] ||
                                  config?.config?.webSearch?.customProviders ||
                                  {};
                          ...
                          {Object.entries(
                            customProvidersLocal[serviceType] ||
                              config?.config?.webSearch?.customProviders ||
                              {}
                          ).map(([providerName, provider]: [string, unknown]) => {
                              ...
                              <Card key={providerName} className="border border-dashed">
                                <CardContent className="space-y-3 pt-4">
                                      ...
                                      <Input
                                        ...
                                        onChange={e => {
                                          ...
                                          if (newName !== providerName && newName) {
                                            const { [providerName]: oldProvider, ...rest } =
                                              currentProviders;
                                  ...
                                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                                    <div className="space-y-2">
                                      <Label htmlFor={`${serviceType}-custom-${providerName}-key`}>
                                        API Key
                                      </Label>
                                      <Input
                                    ...
                                    <div className="space-y-2">
                                      <Label htmlFor={`${serviceType}-custom-${providerName}-url`}>
                                        API URL
                                      </Label>
                                      <Input
                                  ...
                                  <div className="space-y-2">
                                    <Label htmlFor={`${serviceType}-custom-${providerName}-model`}>
                                      Model
                                    </Label>
                                    <Input
                                  ...
                                  <div className="space-y-2">
                                    <Label htmlFor={`${serviceType}-custom-${providerName}-desc`}>
                          ...
                          {Object.keys(
                            ...
                            <div className="text-muted-foreground rounded-lg border border-dashed py-8 text-center text-sm">
                              No custom providers added yet. Click &ldquo;Add Provider&rdquo; to add
                              one.
                            </div>
                          )}
                        </div>
                    ...
                    <DialogContent className="max-w-2xl">
                      ...
                      <div className="space-y-4">
                        <Textarea
                          ...
                          rows={12}
                          value={
                            pendingAdvanced[serviceType] ??
                            JSON.stringify(config?.config || {}, null, 2)
                          }
                          onChange={e =>
                            setPendingAdvanced(prev => ({ ...prev, [serviceType]: e.target.value }))
                          }
                          placeholder="{}"
                        />
                        <Button
