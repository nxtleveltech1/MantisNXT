...
const AISupplierDiscovery = dynamic(() => import('../suppliers/AISupplierDiscovery'), {
...
const MobileAIInterface: React.FC<MobileAIInterfaceProps> = ({
  ...
  const [activeSection, setActiveSection] = useState<'chat' | 'insights' | 'charts' | 'discovery'>(
    'chat'
  );
  ...
  return (
    <div className="relative flex h-screen flex-col bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-100">
            ...
            {navigationTabs.map(tab => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-1 flex-col items-center gap-1 rounded-md px-1 py-2 transition-all duration-200 ${
      ...
      {enableSwipeGestures && (
        <div className="fixed bottom-20 left-1/2 z-30 -translate-x-1/2 transform">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
