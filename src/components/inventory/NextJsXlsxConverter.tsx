...
class SemanticMapper {
  ...
  private static generateSuggestions(mappings: FieldMapping[], headers: string[]): string[] {
    ...
    if (lowConfidenceMappings.length > 0) {
      suggestions.push(
        `${lowConfidenceMappings.length} mappings have low confidence - please review`
      );
    }
  ...
  const validateAndCleanData = useCallback((): DataValidationResult => {
    if (!rawData.length || !headers.length) {
      return {
        isValid: false,
        errors: [
          { row: 0, field: 'file', value: '', message: 'No data to validate', severity: 'error' },
        ],
        warnings: [],
        cleanedData: [],
      ...
      <DialogContent className="flex max-h-[90vh] max-w-6xl flex-col overflow-hidden">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-1 flex-col">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="mapping" disabled={!headers.length}>
              Mapping
            </TabsTrigger>
              ...
              <CardContent className="space-y-4">
                ...
                {file && (
                  <Alert>
                    ...
                    <AlertDescription>
                      File loaded: {file.name} ({Math.round(file.size / 1024)}KB, {rawData.length}{' '}
                      rows)
                    </AlertDescription>
                  </Alert>
                )}
                ...
                <CardContent>
                  <div className="space-y-3">
                          ...
                          <SelectContent>
                            ...
                            {headers.map(header => (
                              <SelectItem key={header} value={header}>
                                {header}
                                {semanticMapping?.mappings.find(
                                  m => m.sourceField === header && m.targetField === field
                                ) && (
                    ...
                    <Button
                      ...
                      disabled={
                        Object.values(manualMappings).filter(Boolean).length <
                          requiredFields.length || processing
                      }
                    >
                  ...
                  <Card>
                    <CardContent className="pt-6 text-center">
                      <div className="text-2xl font-bold text-gray-600">
                        {validationResult.summary.emptyRows}
                      </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              disabled={
                ...
                validationResult.cleanedData.length === 0
              }
            >
