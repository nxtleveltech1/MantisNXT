...
export const LoadingSpinner: React.FC<{
  ...
  return (
    <div
      ...
      aria-label={label}
    >
      <Loader2 className={`${sizeClasses[size]} text-primary animate-spin`} aria-hidden="true" />
      ...
      <div className="overflow-hidden rounded-lg border">
        ...
        <div className="divide-y">
          {[...Array(rows)].map((_, rowIndex) => (
            <div key={`row-${rowIndex}`} className="animate-pulse p-4">
              <div className="flex gap-4">
...
export const ErrorState: React.FC<{
  ...
  showDetails?: boolean;
  error?: Error;
}> = ({ title = 'Error Loading Data', message, onRetry, showDetails = false, error }) => {
  return (
    <div
    ...
    >
      <div className="mb-4">
        <div className="inline-block rounded-full bg-red-50 p-3 dark:bg-red-900/20">
      ...
      {showDetails && error && (
        <details className="mb-4 w-full max-w-md text-left">
          ...
          <pre className="bg-muted mt-2 max-h-40 overflow-auto rounded-lg p-3 text-xs">
            {error.message}
            {error.stack && `\n\n${error.stack}`}
          </pre>
        </details>
...
export const ProgressiveLoader: React.FC<{
  ...
  return (
    <div
      className="mx-auto w-full max-w-md space-y-3 py-8"
      role="progressbar"
    ...
    >
      <div className="flex items-center justify-between text-sm">
        ...
        {showPercentage && <span className="text-muted-foreground">{Math.round(progress)}%</span>}
      </div>
