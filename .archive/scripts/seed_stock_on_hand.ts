...
async function ensureDefaultLocationExists(): Promise<string> {
  try {
    ...
    if (existingLocation.length > 0) {
      console.log(`✓ Using existing location: ${DEFAULT_LOCATION_NAME}`);
...
function generateRandomQty(): number {
  return Math.floor(Math.random() * (INITIAL_QTY_MAX - INITIAL_QTY_MIN + 1)) + INITIAL_QTY_MIN;
}
...
async function seedStockOnHand(): Promise<StockSeedStats> {
  ...
  try {
    console.log('🚀 Starting stock on hand seeding...');

    // Step 1: Ensure default location exists
    console.log('📍 Ensuring default location exists...');
    ...
    console.log('📊 Querying selected products with current prices...');
    ...
    console.log(`✓ Generated ${stockRecords.length} stock records`);
    console.log(`💰 Total inventory value: ${totalValue.toFixed(2)}`);

    // Step 4: Bulk import stock in batches
    console.log('💾 Importing stock records...');
    const batchSize = 100;
    let importedCount = 0;
    ...
    for (let i = 0; i < stockRecords.length; i += batchSize) {
      ...
      try {
        ...
        const progress = Math.round((importedCount / stockRecords.length) * 100);
        console.log(`📊 Progress: ${progress}% (${importedCount}/${stockRecords.length} records)`);
      } catch (error) {
    ...
    console.log('🔍 Verifying stock records...');
    ...
    const verifiedTotalValue = parseFloat(createdStockRecords[0].total_value) || 0;

    console.log(`✓ Verified: ${recordCount} stock records created`);
    console.log(`✓ Verified: Total quantity: ${verifiedTotalQty}`);
    console.log(`✓ Verified: Total value: ${verifiedTotalValue.toFixed(2)}`);
    ...
    console.log('========================================');
    console.log(`📍 Location: ${DEFAULT_LOCATION_NAME} (${locationId})`);
    console.log(`📦 Total Products: ${stats.totalProducts}`);
    console.log(`💰 Total Inventory Value: ${stats.totalInventoryValue.toFixed(2)}`);
    console.log(`📊 Average Quantity: ${stats.averageQty.toFixed(1)}`);
...
async function verifyStockIntegrity(): Promise<void> {
  try {
    console.log('\n🔍 Verifying stock data integrity...');
    ...
    console.log(`✓ Products with stock: ${productsWithStock[0].count}`);
    ...
    if (selectedWithoutStock[0].count > 0) {
      console.warn(`⚠️  ${selectedWithoutStock[0].count} selected products have no stock records`);
    } else {
      console.log(`✓ All selected products have stock records`);
    }
    ...
    if (stockWithoutSelection[0].count > 0) {
      console.warn(
        `⚠️  ${stockWithoutSelection[0].count} stock records are not in active selection`
      );
    } else {
      console.log(`✓ All stock records are in active selection`);
    }
    ...
    console.log('📊 Top suppliers by inventory value:');
    ...
    console.error('❌ Stock integrity verification failed:', error);
  }
}
...
if (require.main === module) {
  seedStockOnHand()
    .then(async stats => {
      ...
      console.log('\n✅ Stock on hand seeding completed successfully!');
      console.log(
        `\n🎯 Next step: Run 'npm run integration:verify' to verify the complete integration`
      );
      process.exit(0);
    })
    .catch(error => {
      console.error('\n❌ Stock on hand seeding failed:', error);
      process.exit(1);
    });
}
