...
export const inventoryItems = pgTable('inventory_items', {
  ...
  return {
    ...
    reservedLeStock: check('inventory_items_reserved_le_stock', `${table.reservedQty.name} <= ${table.stockQty.name}`),
  }
})
...
export const stockMovements = pgTable('stock_movements', {
  ...
  return {
    idxItemCreated: index('idx_movements_item_created').on(table.inventoryItemId, table.createdAt.desc()),
    idxTypeCreated: index('idx_movements_type_created').on(table.movementType, table.createdAt.desc()),
