  ...
  describe('GET /api/inventory', () => {
    ...
    it('should handle sorting by different fields', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/inventory?sortBy=name&sortOrder=asc'
      );
      const response = await GET(request);
      const data = await response.json();
...
describe('/api/inventory/[id]', () => {
  ...
  describe('POST /api/inventory/[id]/stock-adjustment', () => {
    it('should adjust stock successfully with positive adjustment', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/inventory/item_001/stock-adjustment',
        {
          method: 'POST',
    ...
    it('should return 404 for non-existing item', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/inventory/nonexistent/stock-adjustment',
        {
          method: 'POST',
          body: JSON.stringify(adjustmentData),
        }
    ...
    it('should update item status based on new stock level', async () => {
      ...
      const adjustRequest = new NextRequest(
        `http://localhost:3000/api/inventory/${itemId}/stock-adjustment`,
        {
          method: 'POST',
          body: JSON.stringify(adjustmentData),
        }
