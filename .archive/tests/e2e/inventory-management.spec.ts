...
test.describe('Inventory Management E2E Tests', () => {
  ...
  test.describe('Basic Inventory Operations', () => {
    ...
    test('should create new inventory item', async ({ page }) => {
      ...
      await page.fill(
        ...
        testInventoryItem.currentStock.toString()
      );
      await page.fill(
        '[data-testid="reorder-point-input"]',
        testInventoryItem.reorderPoint.toString()
      );
      ...
      await expect(
        page.locator(`[data-testid="inventory-row"]:has-text("${testInventoryItem.sku}")`)
      ).toBeVisible();
    });
    ...
    test('should edit existing inventory item', async ({ page }) => {
      ...
      await expect(
        page.locator(`[data-testid="inventory-row"]:has-text("${newName}")`)
      ).toBeVisible();
    ...
    test('should delete inventory item', async ({ page }) => {
      ...
      await expect(
        page.locator(`[data-testid="inventory-row"]:has-text("${itemSku}")`)
      ).not.toBeVisible();
  ...
  test.describe('Sorting and Pagination', () => {
    test('should sort by column headers', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="sort-name"] [data-testid="sort-asc"]')
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="sort-name"] [data-testid="sort-desc"]')
      ).toBeVisible();
    });
  ...
  test.describe('Bulk Upload Workflow', () => {
    test('should complete successful bulk upload', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="validation-success"]')).toContainText(
        'All items valid'
      );
  ...
  test.describe('Stock Management', () => {
    ...
    test('should show stock alerts for low stock items', async ({ page }) => {
      ...
      if (count > 0) {
        for (let i = 0; i < count; i++) {
          await expect(
            lowStockRows.nth(i).locator('[data-testid="low-stock-indicator"]')
          ).toBeVisible();
        }
      }
    });
  ...
  test.describe('Error Handling', () => {
    ...
    test('should validate form inputs', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="cost-error"]')).toContainText(
  ...
  test.describe('Performance and Accessibility', () => {
    ...
    test('should have proper ARIA labels and roles', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="inventory-table"]')).toHaveAttribute(
        'role',
        'table'
      );
