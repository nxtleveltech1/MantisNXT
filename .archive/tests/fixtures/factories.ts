...
export const InventoryItemFactory = new BaseFactory<TestInventoryItem>((overrides = {}) => {
  ...
  return {
    ...
    tags: faker.helpers.arrayElements(['electronics', 'office', 'hardware', 'premium', 'bulk'], {
      min: 0,
      max: 3,
    }),
...
export const UploadFileFactory = new BaseFactory<TestUploadFile>((overrides = {}) => {
  const type =
    overrides.type || 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
...
export const createInventoryScenario = (
  type: 'low_stock' | 'out_of_stock' | 'overstock' | 'normal'
) => {
  ...
  switch (type) {
    ...
    case 'normal':
      return {
        ...baseItem,
        currentStock: faker.number.int({
          min: baseItem.reorderPoint + 1,
          max: baseItem.maxStock - 10,
        }),
        status: 'active' as const,
      };
  }
