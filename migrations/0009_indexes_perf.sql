...
CREATE INDEX idx_profile_active ON profile(is_active) WHERE is_active = true;
CREATE INDEX idx_profile_last_seen ON profile(last_seen_at DESC) WHERE last_seen_at IS NOT NULL;
...
CREATE INDEX idx_inventory_item_sku ON inventory_item(org_id, sku); -- Composite for uniqueness queries
...
CREATE INDEX idx_inventory_item_low_stock ON inventory_item(org_id) WHERE quantity_on_hand <= reorder_point AND is_active = true;
CREATE INDEX idx_inventory_item_search ON inventory_item USING gin(to_tsvector('english', name || ' ' || COALESCE(description, '') || ' ' || sku));
...
CREATE INDEX idx_purchase_order_number ON purchase_order(org_id, po_number); -- Composite for uniqueness
...
CREATE INDEX idx_ai_dataset_search ON ai_dataset USING gin(to_tsvector('english', name || ' ' || COALESCE(description, '')));
...
CREATE INDEX idx_ai_prompt_search ON ai_prompt_template USING gin(to_tsvector('english', name || ' ' || COALESCE(description, '')));
...
CREATE INDEX idx_customer_search ON customer USING gin(to_tsvector('english', name || ' ' || COALESCE(company, '') || ' ' || COALESCE(email, '')));
...
CREATE INDEX idx_support_ticket_number ON support_ticket(org_id, ticket_number); -- Composite for uniqueness
CREATE INDEX idx_support_ticket_sla ON support_ticket(sla_due_at) WHERE sla_due_at IS NOT NULL AND status IN ('open', 'in_progress');
CREATE INDEX idx_support_ticket_created ON support_ticket(created_at DESC);
CREATE INDEX idx_support_ticket_search ON support_ticket USING gin(to_tsvector('english', title || ' ' || description));
...
CREATE INDEX idx_integration_connector_next_sync ON integration_connector(last_sync_at + (sync_frequency_minutes || ' minutes')::interval) WHERE status = 'active';
...
CREATE INDEX idx_automation_pipeline_next_execution ON automation_pipeline(next_execution_at) WHERE next_execution_at IS NOT NULL AND status = 'active';
...
CREATE INDEX idx_dashboard_search ON dashboard USING gin(to_tsvector('english', name || ' ' || COALESCE(description, '')));
