-- ============================================================================
-- Migration: 0212_sync_preview_schema_restore.sql
-- Purpose : Rebuild sync preview/activity schema after Neon rollback
-- Author  : GPT-5.1 Codex
-- Date    : 2025-11-14
-- ============================================================================

BEGIN;

-- ============================================================================
-- 1. ENUM DEFINITIONS (idempotent with additive fallbacks)
-- ============================================================================

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'sync_type') THEN
        CREATE TYPE sync_type AS ENUM ('woocommerce', 'odoo', 'shopify', 'custom');
    ELSE
        PERFORM 1;
    END IF;
END;
$$;

DO $$
DECLARE
    label text;
    labels text[] := ARRAY['woocommerce', 'odoo', 'shopify', 'custom'];
BEGIN
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'sync_type') THEN
        FOREACH label IN ARRAY labels LOOP
            BEGIN
                EXECUTE format('ALTER TYPE sync_type ADD VALUE IF NOT EXISTS %L', label);
            EXCEPTION
                WHEN duplicate_object THEN
                    NULL;
            END;
        END LOOP;
    END IF;
END;
$$;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'sync_action') THEN
        CREATE TYPE sync_action AS ENUM (
            'preview_delta',
            'sync_start',
            'sync_complete',
            'batch_process',
            'error_recovery',
            'manual_trigger',
            'scheduled_run',
            'partial_resume'
        );
    END IF;
END;
$$;

DO $$
DECLARE
    label text;
    labels text[] := ARRAY[
        'preview_delta',
        'sync_start',
        'sync_complete',
        'batch_process',
        'error_recovery',
        'manual_trigger',
        'scheduled_run',
        'partial_resume'
    ];
BEGIN
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'sync_action') THEN
        FOREACH label IN ARRAY labels LOOP
            BEGIN
                EXECUTE format('ALTER TYPE sync_action ADD VALUE IF NOT EXISTS %L', label);
            EXCEPTION
                WHEN duplicate_object THEN
                    NULL;
            END;
        END LOOP;
    END IF;
END;
$$;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'sync_entity_type') THEN
        CREATE TYPE sync_entity_type AS ENUM (
            'customers',
            'products',
            'orders',
            'invoices',
            'inventory',
            'suppliers',
            'categories',
            'attributes'
        );
    END IF;
END;
$$;

DO $$
DECLARE
    label text;
    labels text[] := ARRAY[
        'customer', 'customers',
        'product', 'products',
        'order', 'orders',
        'invoice', 'invoices',
        'inventory', 'inventories',
        'supplier', 'suppliers',
        'category', 'categories',
        'attribute', 'attributes'
    ];
BEGIN
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'sync_entity_type') THEN
        FOREACH label IN ARRAY labels LOOP
            BEGIN
                EXECUTE format('ALTER TYPE sync_entity_type ADD VALUE IF NOT EXISTS %L', label);
            EXCEPTION
                WHEN duplicate_object THEN
                    NULL;
            END;
        END LOOP;
    END IF;
END;
$$;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'sync_status_type') THEN
        CREATE TYPE sync_status_type AS ENUM (
            'pending',
            'in_progress',
            'completed',
            'completed_with_errors',
            'failed',
            'cancelled',
            'paused',
            'timeout'
        );
    END IF;
END;
$$;

DO $$
DECLARE
    label text;
    labels text[] := ARRAY[
        'pending',
        'in_progress',
        'completed',
        'completed_with_errors',
        'failed',
        'cancelled',
        'paused',
        'timeout'
    ];
BEGIN
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'sync_status_type') THEN
        FOREACH label IN ARRAY labels LOOP
            BEGIN
                EXECUTE format('ALTER TYPE sync_status_type ADD VALUE IF NOT EXISTS %L', label);
            EXCEPTION
                WHEN duplicate_object THEN
                    NULL;
            END;
        END LOOP;
    END IF;
END;
$$;

-- Compat view so code querying organizations keeps working
DROP VIEW IF EXISTS organizations CASCADE;
CREATE VIEW organizations AS
SELECT * FROM organization;

-- ============================================================================
-- 3. TABLES + INDEXES
-- ============================================================================

DROP TABLE IF EXISTS sync_preview_cache CASCADE;
CREATE TABLE sync_preview_cache (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id UUID NOT NULL REFERENCES organization(id) ON DELETE CASCADE,
    sync_type sync_type NOT NULL,
    entity_type sync_entity_type NOT NULL,
    sync_id VARCHAR(255),
    delta_data JSONB NOT NULL DEFAULT '{}'::jsonb,
    computed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    expires_at TIMESTAMPTZ NOT NULL DEFAULT (NOW() + INTERVAL '1 hour'),
    computed_by UUID,
    cache_key VARCHAR(255),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT sync_preview_unique UNIQUE (org_id, sync_type, entity_type)
);

CREATE INDEX idx_sync_preview_org_type ON sync_preview_cache(org_id, sync_type, entity_type);
CREATE INDEX idx_sync_preview_expires ON sync_preview_cache(expires_at);
CREATE INDEX idx_sync_preview_key ON sync_preview_cache(cache_key) WHERE cache_key IS NOT NULL;

DROP TABLE IF EXISTS sync_progress CASCADE;
CREATE TABLE sync_progress (
    id UUID PRIMARY KEY,
    org_id UUID NOT NULL REFERENCES organization(id) ON DELETE CASCADE,
    sync_id VARCHAR(255) NOT NULL,
    job_id VARCHAR(255),
    entity_type sync_entity_type,
    total_items INTEGER NOT NULL DEFAULT 0,
    processed_count INTEGER NOT NULL DEFAULT 0,
    failed_count INTEGER NOT NULL DEFAULT 0,
    status TEXT NOT NULL DEFAULT 'running',
    metadata JSONB NOT NULL DEFAULT '{}'::jsonb,
    started_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    completed_at TIMESTAMPTZ,
    initiated_by UUID,
    CONSTRAINT sync_progress_counts CHECK (processed_count >= 0 AND failed_count >= 0),
    CONSTRAINT sync_progress_unique UNIQUE (org_id, sync_id)
);

CREATE INDEX idx_sync_progress_org_job ON sync_progress(org_id, job_id);
CREATE INDEX idx_sync_progress_org_sync ON sync_progress(org_id, sync_id);
CREATE INDEX idx_sync_progress_org_status ON sync_progress(org_id, status, updated_at DESC);

DROP TABLE IF EXISTS sync_activity_log CASCADE;
CREATE TABLE sync_activity_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id UUID NOT NULL REFERENCES organization(id) ON DELETE CASCADE,
    sync_id VARCHAR(255),
    entity_type sync_entity_type,
    activity_type TEXT NOT NULL,
    status TEXT NOT NULL,
    details JSONB DEFAULT '{}'::jsonb,
    user_id UUID,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX idx_sync_activity_org ON sync_activity_log(org_id, created_at DESC);
CREATE INDEX idx_sync_activity_sync ON sync_activity_log(sync_id, created_at DESC);

-- Back-compat view for /api/v1/integrations/sync/activity route
DROP VIEW IF EXISTS integration_sync_activity CASCADE;
CREATE VIEW integration_sync_activity AS
SELECT
    l.id,
    l.org_id,
    COALESCE(l.activity_type, 'sync') AS action,
    l.status,
    COALESCE((l.details ->> 'started_at')::timestamptz, l.created_at) AS started_at,
    COALESCE((l.details ->> 'finished_at')::timestamptz, l.created_at) AS finished_at,
    COALESCE(l.details ->> 'error', l.details ->> 'error_message') AS error_message
FROM sync_activity_log l;

-- ============================================================================
-- 4. TRIGGERS + RLS
-- ============================================================================

CREATE OR REPLACE FUNCTION sync_preview_touch_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at := NOW();
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_sync_preview_touch ON sync_preview_cache;
CREATE TRIGGER trg_sync_preview_touch
    BEFORE UPDATE ON sync_preview_cache
    FOR EACH ROW
    EXECUTE FUNCTION sync_preview_touch_updated_at();

ALTER TABLE sync_preview_cache ENABLE ROW LEVEL SECURITY;
ALTER TABLE sync_progress ENABLE ROW LEVEL SECURITY;
ALTER TABLE sync_activity_log ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS preview_cache_rls ON sync_preview_cache;
CREATE POLICY preview_cache_rls ON sync_preview_cache
    USING (org_id = current_setting('app.current_org_id', true)::uuid)
    WITH CHECK (org_id = current_setting('app.current_org_id', true)::uuid);

DROP POLICY IF EXISTS sync_progress_rls ON sync_progress;
CREATE POLICY sync_progress_rls ON sync_progress
    USING (org_id = current_setting('app.current_org_id', true)::uuid)
    WITH CHECK (org_id = current_setting('app.current_org_id', true)::uuid);

DROP POLICY IF EXISTS sync_activity_rls ON sync_activity_log;
CREATE POLICY sync_activity_rls ON sync_activity_log
    USING (org_id = current_setting('app.current_org_id', true)::uuid)
    WITH CHECK (org_id = current_setting('app.current_org_id', true)::uuid);

-- ============================================================================
-- 5. MIGRATION RECORD
-- ============================================================================

INSERT INTO schema_migrations (migration_name)
VALUES ('0212_sync_preview_schema_restore')
ON CONFLICT (migration_name) DO NOTHING;

COMMIT;

-- ============================================================================
-- End of migration
-- ============================================================================

